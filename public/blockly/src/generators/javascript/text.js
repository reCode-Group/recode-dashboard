...
export function text_charAt(block, generator) {
    ...
    switch (where) {
        ...
        case 'RANDOM': {
            const functionName = generator.provideFunction_('textRandomLetter', `
...
export function text_getSubstring(block, generator) {
    ...
    const requiresLengthCall = where1 !== 'FROM_END' &&
        where1 !== 'LAST' &&
        where2 !== 'FROM_END' &&
        where2 !== 'LAST';
...
export function text_changeCase(block, generator) {
    ...
    if (operator) {
        ...
        const functionName = generator.provideFunction_('textToTitleCase', `
