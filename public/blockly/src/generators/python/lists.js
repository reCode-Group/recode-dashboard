...
export function lists_repeat(block, generator) {
    ...
    const times = generator.valueToCode(block, 'NUM', Order.MULTIPLICATIVE) || '0';
...
export function lists_getIndex(block, generator) {
    ...
    switch (where) {
        ...
        case 'RANDOM':
            generator.definitions_['import_random'] =
                'import random';
            if (mode === 'GET') {
    ...
    switch (where2) {
        ...
        case 'FROM_END':
            ...
            if (!stringUtils.isNumber(String(at2))) {
                generator.definitions_['import_sys'] =
                    'import sys';
                at2 += ' or sys.maxsize';
            }
            else if (at2 === 0) {
...
`);
    const code = sortFunctionName + '(' + list + ', "' + type + '", ' + reverse + ')';
    return [code, Order.FUNCTION_CALL];
}
export function lists_split(block, generator) {
    ...
    let code;
    if (mode === 'SPLIT') {
        const value_input = generator.valueToCode(block, 'INPUT', Order.MEMBER) || "''";
        const value_delim = generator.valueToCode(block, 'DELIM', Order.NONE);
        code = value_input + '.split(' + value_delim + ')';
    }
    else if (mode === 'JOIN') {
        const value_input = generator.valueToCode(block, 'INPUT', Order.NONE) || '[]';
        const value_delim = generator.valueToCode(block, 'DELIM', Order.MEMBER) || "''";
