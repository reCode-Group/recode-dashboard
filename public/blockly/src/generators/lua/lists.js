...
export function lists_getIndex(block, generator) {
    ...
    if ((where === 'LAST' || where === 'FROM_END' || where === 'RANDOM') &&
        ...
        if (mode === 'REMOVE') {
            ...
            let functionName;
            if (mode === 'GET') {
                functionName = generator.provideFunction_('list_get_' + where.toLowerCase(), [
                    'function ' +
                ...
                // `mode` === 'GET_REMOVE'
                functionName = generator.provideFunction_('list_remove_' + where.toLowerCase(), [
                    'function ' +
                        generator.FUNCTION_NAME_PLACEHOLDER_ +
                        '(t' +
                        // The value for 'FROM_END' and'FROM_START' depends on `at` so
        ...
        // once.
        const atOrder = mode === 'GET' && where === 'FROM_END' ? Order.ADDITIVE : Order.NONE;
        let at = generator.valueToCode(block, 'AT', atOrder) || '1';
...
export function lists_getSublist(block, generator) {
    ...
    const functionName = generator.provideFunction_('list_sublist_' + where1.toLowerCase() + '_' + where2.toLowerCase(), `
