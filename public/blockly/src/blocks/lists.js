...
const LISTS_CREATE_WITH = {
    ...
    init: function () {
        ...
        this.setMutator(new MutatorIcon(['lists_create_with_item'], this)); // BUG(#6905)
    ...
    updateShape_: function () {
        if (this.itemCount_ && this.getInput('EMPTY')) {
            ...
            this.appendDummyInput('EMPTY').appendField(Msg['LISTS_CREATE_EMPTY_TITLE']);
        }
        // Add new inputs.
        for (let i = 0; i < this.itemCount_; i++) {
...
const LISTS_INDEXOF = {
    ...
    init: function () {
        ...
        if (!operatorsDropdown)
            throw new Error('field_dropdown not found');
        this.appendValueInput('FIND').appendField(operatorsDropdown, 'END');
        this.setInputsInline(true);
        this.setTooltip(() => {
            return Msg['LISTS_INDEX_OF_TOOLTIP'].replace('%1', this.workspace.options.oneBasedIndex ? '0' : '-1');
        });
    },
};
    ...
    init: function () {
        ...
        this.setTooltip(() => {
            ...
            if (where === 'FROM_START' || where === 'FROM_END') {
                tooltip +=
                    '  ' +
                        Msg['LISTS_INDEX_FROM_START_TOOLTIP'].replace('%1', this.workspace.options.oneBasedIndex ? '#1' : '#0');
            }
            return tooltip;
        });
    },
