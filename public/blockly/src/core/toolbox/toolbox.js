...
export class Toolbox extends DeleteArea {
    ...
    attachEvents_(container, contentsContainer) {
        ...
        const clickEvent = browserEvents.conditionalBind(container, 'pointerdown', this, this.onClick_, 
        /* opt_noCaptureIdentifier */ false);
        this.boundEvents_.push(clickEvent);
        const keyDownEvent = browserEvents.conditionalBind(contentsContainer, 'keydown', this, this.onKeyDown_, 
        /* opt_noCaptureIdentifier */ false);
    ...
    onKeyDown_(e) {
        ...
        switch (e.key) {
            ...
            case ' ':
                if (this.selectedItem_ && this.selectedItem_.isCollapsible()) {
                    const collapsibleItem = this.selectedItem_;
        ...
        if (!handled && this.selectedItem_) {
            ...
            // should be part of.
            if (this.selectedItem_.onKeyDown) {
    ...
    createFlyout_() {
        ...
        if (workspace.horizontalLayout) {
            FlyoutClass = registry.getClassFromOptions(registry.Type.FLYOUTS_HORIZONTAL_TOOLBOX, workspace.options, true);
        }
        else {
            FlyoutClass = registry.getClassFromOptions(registry.Type.FLYOUTS_VERTICAL_TOOLBOX, workspace.options, true);
        }
     ...
     */
    createToolboxItem(toolboxItemDef, fragment) {
        ...
        if (registryName.toUpperCase() === 'CATEGORY' &&
            toolbox.isCategoryCollapsible(toolboxItemDef)) {
            registryName = CollapsibleToolboxCategory.registrationName;
        }
        const ToolboxItemClass = registry.getClass(registry.Type.TOOLBOX_ITEM, registryName.toLowerCase());
        if (ToolboxItemClass) {
    ...
    addToolboxItem_(toolboxItem) {
        ...
        if (toolboxItem.isCollapsible()) {
            const collapsibleItem = toolboxItem;
    ...
    deselectItem_(item) {
        ...
        aria.setState(this.contentsDiv_, aria.State.ACTIVEDESCENDANT, '');
    }
    /**
    ...
    selectItem_(oldItem, newItem) {
        ...
        aria.setState(this.contentsDiv_, aria.State.ACTIVEDESCENDANT, newItem.getId());
    }
    /**
     ...
     * @param newItem The newly selected toolbox item.
     */
    fireSelectEvent(oldItem, newItem) {
        ...
        const event = new (eventUtils.get(EventType.TOOLBOX_ITEM_SELECT))(oldElement, newElement, this.workspace_.id);
    ...
    getFocusableElement() {
        if (!this.HtmlDiv)
            throw Error('Toolbox DOM has not yet been created.');
        return this.HtmlDiv;
    }
...
.blocklyToolbox {
  ...
  padding: 16px;
  position: absolute;
  box-shadow: 0px 0px 35px 0px rgba(174, 174, 174, 0.2);
