 ...
 */
const WORKSPACE_AT_BLOCK_CAPACITY_DISABLED_REASON = 'WORKSPACE_AT_BLOCK_CAPACITY';
const BLOCK_TYPE = 'block';
/**
 * Class responsible for creating blocks for flyouts.
 */
export class BlockFlyoutInflater {
    /**
    ...
    load(state, flyout) {
        ...
        const block = this.createBlock(state, flyout.getWorkspace());
        if (!block.isEnabled()) {
    ...
    createBlock(blockDefinition, workspace) {
        ...
        if (blockDefinition['blockxml']) {
            const xml = (typeof blockDefinition['blockxml'] === 'string'
            ...
            if (blockDefinition['disabledReasons'] === undefined &&
            ...
            block = blocks.appendInternal(blockDefinition, workspace);
        }
        return block;
    }
    /**
    ...
    gapForItem(state, defaultGap) {
        ...
        if (blockState['gap']) {
            ...
            const xml = (typeof blockState['blockxml'] === 'string'
     ...
     */
    updateStateBasedOnCapacity(block) {
        const enable = this.flyout?.targetWorkspace?.isCapacityAvailable(common.getBlockTypeCounts(block));
    ...
    addBlockListeners(block) {
        ...
        blockListeners.push(browserEvents.conditionalBind(block.getSvgRoot(), 'pointerdown', block, (e) => {
        ...
        blockListeners.push(browserEvents.bind(block.getSvgRoot(), 'pointermove', null, () => {
            if (!this.flyout?.targetWorkspace?.isDragging()) {
                block.addSelect();
            }
        }));
        blockListeners.push(browserEvents.bind(block.getSvgRoot(), 'pointerleave', null, () => {
            if (!this.flyout?.targetWorkspace?.isDragging()) {
                block.removeSelect();
            }
        }));
     ...
     */
    filterFlyoutBasedOnCapacity(event) {
        if (!this.flyout ||
            (event &&
...
registry.register(registry.Type.FLYOUT_INFLATER, BLOCK_TYPE, BlockFlyoutInflater);
//# sourceMappingURL=block_flyout_inflater.js.map