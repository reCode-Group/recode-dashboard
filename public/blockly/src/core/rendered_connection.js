...
export class RenderedConnection extends Connection {
     ...
     * @internal
     */
    bumpAwayFrom(superiorConnection, initiatedByThis = false) {
        if (this.sourceBlock_.workspace.isDragging()) {
            // Don't move blocks around while the user is doing the same.
            return;
        }
        let offsetX = config.snapRadius + Math.floor(Math.random() * BUMP_RANDOMNESS);
        let offsetY = config.snapRadius + Math.floor(Math.random() * BUMP_RANDOMNESS);
    ...
    moveTo(x, y) {
        ...
        else if (this.trackedState === RenderedConnection.TrackedState.TRACKED &&
            moved) {
            this.db.removeConnection(this, this.y);
            this.db.addConnection(this, y);
            updated = true;
    ...
    moveToOffset(blockTL) {
        return this.moveTo(blockTL.x + this.offsetInBlock.x, blockTL.y + this.offsetInBlock.y);
    }
    /**
    ...
    tightenEfficiently() {
        ...
        if (!target || !block)
            return;
        const offset = Coordinate.difference(this.offsetInBlock, target.offsetInBlock);
     ...
     * @internal
     */
    onFailedConnect(superiorConnection) {
        ...
        if (eventUtils.getRecordUndo()) {
            ...
            setTimeout(function () {
                if (!block.isDisposed() && !block.getParent()) {
                    ...
                    this.bumpAwayFrom(superiorConnection);
     ...
     */
    disconnectInternal(setParent = true) {
        const { parentConnection, childConnection } = this.getParentAndChildConnections();
        ...
        const child = childConnection.getSourceBlock();
    ...
    connect_(childConnection) {
        ...
        const renderedChildConnection = childConnection;
    ...
    onCheckChanged_() {
        ...
        if (this.isConnected() &&
            (!this.targetConnection ||
                !this.getConnectionChecker().canConnect(this, this.targetConnection, false))) {
     ...
     */
    showContextMenu(e) {
        const menuOptions = ContextMenuRegistry.registry.getContextMenuOptions({ focusedNode: this }, e);
        ...
        if (e instanceof PointerEvent) {
            ...
            const connectionScreenCoords = svgMath.wsToScreenCoordinates(workspace, connectionWSCoords);
...
(function (RenderedConnection) {
    ...
    (function (TrackedState) {
        TrackedState[TrackedState["WILL_TRACK"] = -1] = "WILL_TRACK";
        TrackedState[TrackedState["UNTRACKED"] = 0] = "UNTRACKED";
        TrackedState[TrackedState["TRACKED"] = 1] = "TRACKED";
    })(TrackedState = RenderedConnection.TrackedState || (RenderedConnection.TrackedState = {}));
})(RenderedConnection || (RenderedConnection = {}));
export const TrackedState = RenderedConnection.TrackedState;
//# sourceMappingURL=rendered_connection.js.map