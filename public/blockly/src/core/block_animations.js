...
export function disposeUiEffect(block) {
    ...
    if (block.getDescendants(false).length > 100)
        return;
 ...
 */
function disposeUiStep(clone, rect, rtl, start) {
...
export function connectionUiEffect(block) {
    ...
    const ripple = dom.createSvgElement(Svg.CIRCLE, {
        'cx': xy.x,
        'cy': xy.y,
        'r': 0,
        'fill': 'none',
    ...
    const scaleAnimation = dom.createSvgElement(Svg.ANIMATE, {
        'id': 'animationCircle',
        'begin': 'indefinite',
        'attributeName': 'r',
        'dur': '150ms',
    ...
    const opacityAnimation = dom.createSvgElement(Svg.ANIMATE, {
        'id': 'animationOpacity',
        'begin': 'indefinite',
 ...
 */
function disconnectUiStep(block, magnitude, start, step) {
...
//# sourceMappingURL=block_animations.js.map