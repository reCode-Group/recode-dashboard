...
import { BadConnectionCheck, MissingBlockType, MissingConnection, RealChildOfShadow, UnregisteredIcon, } from './exceptions.js';
 ...
 */
export function save(block, { addCoordinates = false, addInputBlocks = true, addNextBlocks = true, doFullSerialization = true, saveIds = true, } = {}) {
 ...
 */
function saveExtraState(block, state, doFullSerialization) {
 ...
 */
function saveIcons(block, state, doFullSerialization) {
    const icons = Object.create(null);
    for (const icon of block.getIcons()) {
        if (isSerializable(icon)) {
            const state = icon.saveState(doFullSerialization);
            if (state)
                icons[icon.getType().toString()] = state;
        }
    }
 ...
 */
function saveInputBlocks(block, state, doFullSerialization, saveIds) {
    const inputs = Object.create(null);
    for (let i = 0; i < block.inputList.length; i++) {
        const input = block.inputList[i];
        if (!input.connection)
            continue;
        const connectionState = saveConnection(input.connection, doFullSerialization, saveIds);
        if (connectionState) {
 ...
 * @param doFullSerialization Whether or not to do full serialization.
 */
function saveNextBlocks(block, state, doFullSerialization, saveIds) {
    if (!block.nextConnection) {
        return;
    }
    const connectionState = saveConnection(block.nextConnection, doFullSerialization, saveIds);
    if (connectionState) {
 ...
 * @param doFullSerialization Whether or not to do full serialization.
 */
function saveConnection(connection, doFullSerialization, saveIds) {
 ...
 * @internal
 */
export function appendInternal(state, workspace, { parentConnection = undefined, isShadow = false, recordUndo = false, } = {}) {
 ...
 * @returns The block that was just appended.
 */
function appendPrivate(state, workspace, { parentConnection = undefined, isShadow = false, } = {}) {
    if (!state['type']) {
 ...
 */
function checkNewVariables(workspace, originalVariables) {
    if (eventUtils.isEnabled()) {
        const newVariables = Variables.getAddedVariables(workspace, originalVariables);
        // Fire a VarCreate event for each (if any) new variable created.
 ...
 */
function loadCoords(block, state) {
 ...
 */
function tryToConnectParent(parentConnection, child, state) {
    if (!parentConnection) {
        return;
    }
    ...
    if (!connected) {
        ...
        throw new BadConnectionCheck(checker.getErrorMessage(checker.canConnectWithReason(childConnection, parentConnection, false), childConnection, parentConnection), parentConnection.type === inputTypes.VALUE
            ? 'output connection'
            : 'previous connection', child, state);
    }
}
/**
...
function loadIcons(block, state) {
    ...
    for (const iconType of iconTypes) {
        ...
        if (!icon) {
            ...
            if (!constructor)
                throw new UnregisteredIcon(iconType, block, state);
...
function loadFields(block, state) {
    ...
    for (let i = 0; i < keys.length; i++) {
        ...
        if (!field) {
            console.warn(`Ignoring non-existant field ${fieldName} in block ${block.type}`);
            continue;
        }
...
function loadConnection(connection, connectionState) {
    ...
    if (connectionState['block']) {
        appendPrivate(connectionState['block'], connection.getSourceBlock().workspace, { parentConnection: connection });
    }
}
 ...
 */
function initBlock(block, rendered) {
    if (rendered) {
        const blockSvg = block;
        // Adding connections to the connection db is expensive. This defers that
        // operation to decrease load time.
