    ...
    const disableOption = {
        ...
        preconditionFn(scope) {
            ...
            if (!block.isInFlyout &&
                ...
                const isDisabledForOtherReason = disabledReasons.size >
...
export function registerCommentCreate() {
    const createOption = {
        ...
        callback: (scope, menuOpenEvent, menuSelectEvent, location) => {
            ...
            comment.moveTo(pixelsToWorkspaceCoords(new Coordinate(location.x, location.y), workspace));
 ...
 */
function pixelsToWorkspaceCoords(pixelCoord, workspace) {
    ...
    const clientOffsetPixels = new Coordinate(pixelCoord.x - boundingRect.left, pixelCoord.y - boundingRect.top);
    // The offset in pixels between the main workspace's origin and the upper
    // left corner of the injection div.
    const mainOffsetPixels = workspace.getOriginOffsetInPixels();
    // The position of the new comment in pixels relative to the origin of the
    // main workspace.
    const finalOffset = Coordinate.difference(clientOffsetPixels, mainOffsetPixels);
