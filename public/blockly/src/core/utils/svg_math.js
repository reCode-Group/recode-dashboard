 ...
 */
const XY_STYLE_REGEX = /transform:\s*translate(?:3d)?\(\s*([-+\d.e]+)\s*px([ ,]\s*([-+\d.e]+)\s*px)?/;
/**
...
export function getViewportBBox() {
    ...
    return new Rect(scrollOffset.y, document.documentElement.clientHeight + scrollOffset.y, scrollOffset.x, document.documentElement.clientWidth + scrollOffset.x);
}
/**
...
export function screenToWsCoordinates(ws, screenCoordinates) {
    ...
    const clientOffsetPixels = new Coordinate(screenX - boundingRect.left, screenY - boundingRect.top);
    // The offset in pixels between the main workspace's origin and the upper
    // left corner of the injection div.
    const mainOffsetPixels = ws.getOriginOffsetInPixels();
    // The position of the new comment in pixels relative to the origin of the
    // main workspace.
    const finalOffsetPixels = Coordinate.difference(clientOffsetPixels, mainOffsetPixels);
 ...
 */
export function wsToScreenCoordinates(ws, workspaceCoordinates) {
    ...
    return new Coordinate(screenX + boundingRect.left + mainOffset.x, screenY + boundingRect.top + mainOffset.y);
}
export const TEST_ONLY = {
    XY_REGEX,
    XY_STYLE_REGEX,
};
//# sourceMappingURL=svg_math.js.map