...
export class FocusableTreeTraverser {
    ...
    static findFocusedNode(tree) {
        ...
        if (!rootNode.canBeFocused())
            return null;
        const root = rootNode.getFocusableElement();
        if (dom.hasClass(root, FocusableTreeTraverser.ACTIVE_CLASS_NAME) ||
        ...
        if (activeEl instanceof HTMLElement || activeEl instanceof SVGElement) {
            const active = FocusableTreeTraverser.findFocusableNodeFor(activeEl, tree);
            if (active)
                return active;
        }
        ...
        if (passiveEl instanceof HTMLElement || passiveEl instanceof SVGElement) {
            const passive = FocusableTreeTraverser.findFocusableNodeFor(passiveEl, tree);
            if (passive)
                return passive;
        }
        return null;
...
FocusableTreeTraverser.ACTIVE_CLASS_NAME = 'blocklyActiveFocus';
FocusableTreeTraverser.PASSIVE_CSS_CLASS_NAME = 'blocklyPassiveFocus';
FocusableTreeTraverser.ACTIVE_FOCUS_NODE_CSS_SELECTOR = `.${FocusableTreeTraverser.ACTIVE_CLASS_NAME}`;
FocusableTreeTraverser.PASSIVE_FOCUS_NODE_CSS_SELECTOR = `.${FocusableTreeTraverser.PASSIVE_CSS_CLASS_NAME}`;
//# sourceMappingURL=focusable_tree_traverser.js.map