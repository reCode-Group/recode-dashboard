...
import { isBlockChange, isBlockCreate, isBlockDelete, isBlockFieldIntermediateChange, isBubbleOpen, } from './events/predicates.js';
import { EventType } from './events/type.js';
import * as eventUtils from './events/utils.js';
import { UnattachedFieldError } from './field.js';
import { MutatorIcon } from './icons.js';
import { isLegacyProcedureCallBlock, isLegacyProcedureDefBlock, } from './interfaces/i_legacy_procedure_blocks.js';
...
export function isNameUsed(name, workspace, opt_exclude) {
    ...
    for (const model of workspace.getProcedureMap().getProcedures()) {
        if (model === excludeModel)
            continue;
        if (Names.equals(model.getName(), name))
            return true;
    }
    return false;
}
...
export function rename(name) {
    ...
    if (oldName !== name && oldName !== legalName) {
        ...
        for (let i = 0; i < blocks.length; i++) {
            ...
            const procedureBlock = blocks[i];
            if (procedureBlock.renameProcedure) {
                procedureBlock.renameProcedure(oldName, legalName);
            }
        }
    }
    return legalName;
...
function xmlFlyoutCategory(workspace) {
    ...
    if (Blocks['procedures_defnoreturn']) {
        ...
        nameField.appendChild(utilsXml.createTextNode(Msg['PROCEDURES_DEFNORETURN_PROCEDURE']));
    ...
    if (Blocks['procedures_defreturn']) {
        ...
        const nameField = utilsXml.createElement('field');
        nameField.setAttribute('name', 'NAME');
        nameField.appendChild(utilsXml.createTextNode(Msg['PROCEDURES_DEFRETURN_PROCEDURE']));
        block.appendChild(nameField);
        xmlList.push(block);
    }
...
export function flyoutCategory(workspace, useXml = true) {
    if (useXml) {
        deprecation.warn('The XML return value of Blockly.Procedures.flyoutCategory()', 'v12', 'v13', 'the same method, but handle a return type of FlyoutItemInfo[] (JSON) instead.');
 ...
 */
function updateMutatorFlyout(workspace) {
    const usedNames = [];
    ...
    const nameField = utilsXml.createElement('field');
    nameField.setAttribute('name', 'NAME');
    const argValue = Variables.generateUniqueNameFromOptions(DEFAULT_ARG, usedNames);
...
export function mutatorOpenListener(e) {
    ...
    const block = common
        ...
        .getBlockById(bubbleEvent.blockId);
    const type = block.type;
    if (type !== 'procedures_defnoreturn' && type !== 'procedures_defreturn') {
        return;
    }
    const workspace = block.getIcon(MutatorIcon.TYPE).getWorkspace();
...
function mutatorChangeListener(e) {
    ...
    const workspace = common.getWorkspaceById(workspaceId);
    updateMutatorFlyout(workspace);
}
/**
 ...
 *     procedure name.
 */
function blockIsModernCallerFor(block, procName) {
    return (isProcedureBlock(block) &&
...
export function mutateCallers(defBlock) {
    ...
    for (let i = 0, caller; (caller = callers[i]); i++) {
        ...
        if (oldMutation !== newMutation) {
            ...
            eventUtils.fire(new (eventUtils.get(EventType.BLOCK_CHANGE))(caller, 'mutation', null, oldMutation, newMutation));
...
export { isProcedureBlock, ObservableProcedureMap, };
