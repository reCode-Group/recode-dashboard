...
export function inject(container, opt_options) {
    ...
    browserEvents.conditionalBind(dropDownDiv.getContentDiv(), 'keydown', null, onKeyDown);
 ...
 * @returns Newly created main workspace.
 */
function createMainWorkspace(injectionDiv, svg, options) {
 ...
 */
function init(mainWorkspace) {
    const options = mainWorkspace.options;
    const svg = mainWorkspace.getParentSvg();
    // Suppress the browser's context menu.
    browserEvents.conditionalBind(svg.parentNode, 'contextmenu', null, function (e) {
        if (!browserEvents.isTargetInput(e)) {
            e.preventDefault();
        }
    });
    const workspaceResizeHandler = browserEvents.conditionalBind(window, 'resize', null, function () {
    ...
    if (options.moveOptions && options.moveOptions.scrollbars) {
        ...
        mainWorkspace.scrollbar = new ScrollbarPair(mainWorkspace, horizontalScroll, verticalScroll, 'blocklyMainWorkspaceScrollbar');
 ...
 */
function loadSounds(pathToMedia, workspace) {
    ...
    soundBinds.push(browserEvents.conditionalBind(document, 'pointermove', null, unbindSounds, true));
    soundBinds.push(browserEvents.conditionalBind(document, 'touchstart', null, unbindSounds, true));
}
