...
export class InsertionMarkerPreviewer {
    ...
    shouldUseMarkerPreview(_draggedConn, staticConn) {
        return (staticConn.type === ConnectionType.PREVIOUS_STATEMENT ||
            staticConn.type === ConnectionType.NEXT_STATEMENT ||
            !(this.workspace.getRenderer() instanceof ZelosRenderer));
    }
    previewMarker(draggedConn, staticConn) {
        ...
        renderManagement.finishQueuedRenders().then(() => {
            if (marker.isDeadOrDying())
                return;
            eventUtils.disable();
            try {
                // Position so that the existing block doesn't move.
                marker?.positionNearConnection(markerConn, originalOffsetToTarget, originalOffsetInBlock);
    ...
    createInsertionMarker(origBlock) {
        ...
        if (!blockJson) {
            throw new Error(`Failed to serialize source block. ${origBlock.toDevString()}`);
        }
        const result = blocks.append(blockJson, this.workspace);
    ...
    hideInsertionMarker(markerConn) {
...
registry.register(registry.Type.CONNECTION_PREVIEWER, registry.DEFAULT, InsertionMarkerPreviewer);
//# sourceMappingURL=insertion_marker_previewer.js.map