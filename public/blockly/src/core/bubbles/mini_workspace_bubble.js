...
export class MiniWorkspaceBubble extends Bubble {
    /** @internal */
    constructor(workspaceOptions, workspace, anchor, ownerRect) {
        ...
        if (options.languageTree) {
            background.insertBefore(this.miniWorkspace.addFlyout(Svg.G), this.miniWorkspace.getCanvas());
    ...
    validateWorkspaceOptions(options) {
        if (options.hasCategories) {
            throw new Error('The miniworkspace bubble does not support toolboxes with categories');
        }
        ...
        if (options.moveOptions.scrollbars ||
            ...
            throw new Error('The miniworkspace bubble does not scrolling/moving the workspace');
        }
        if (options.horizontalLayout) {
            throw new Error('The miniworkspace bubble does not support horizontal layouts');
        }
    }
    onWorkspaceChange() {
    ...
    updateBubbleSize() {
        ...
        if (Math.abs(currSize.width - newSize.width) <
        ...
        this.setSize(new Size(newSize.width + Bubble.DOUBLE_BORDER, newSize.height + Bubble.DOUBLE_BORDER), this.autoLayout);
 ...
 * resizing the bubble.
 */
MiniWorkspaceBubble.MINIMUM_VIEW_CHANGE = 10;
/**
 * An arbitrary margin of whitespace to put around the blocks in the
 * workspace.
 */
MiniWorkspaceBubble.MARGIN = Bubble.DOUBLE_BORDER * 3;
//# sourceMappingURL=mini_workspace_bubble.js.map