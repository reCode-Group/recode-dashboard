...
export class Bubble {
     ...
     */
    constructor(workspace, anchor, ownerRect, overriddenFocusableElement) {
        ...
        this.relativeLeft = 0;
        this.dragStrategy = new BubbleDragStrategy(this, this.workspace);
        this.id = idGenerator.getNextUniqueId();
        this.svgRoot = dom.createSvgElement(Svg.G, { 'class': 'blocklyBubble' }, workspace.getBubbleCanvas());
        const embossGroup = dom.createSvgElement(Svg.G, { 'class': 'blocklyEmboss' }, this.svgRoot);
        this.tail = dom.createSvgElement(Svg.PATH, { 'class': 'blocklyBubbleTail' }, embossGroup);
        this.background = dom.createSvgElement(Svg.RECT, {
        ...
        browserEvents.conditionalBind(this.background, 'pointerdown', this, this.onMouseDown);
    }
     ...
     */
    setAnchorLocation(anchor, relayout = false) {
        this.anchor = anchor;
        if (relayout) {
    ...
    setSize(size, relayout = false) {
        size.width = Math.max(size.width, _a.MIN_SIZE);
        size.height = Math.max(size.height, _a.MIN_SIZE);
        this.size = size;
    ...
    positionByRect(rect = new Rect(0, 0, 0, 0)) {
        ...
        const closerPosition = rect.getWidth() < rect.getHeight() ? endPosition : bottomPosition;
        const fartherPosition = rect.getWidth() < rect.getHeight() ? bottomPosition : endPosition;
        ...
        const fartherPositionOverlap = this.getOverlap(fartherPosition, viewMetrics);
        // Set the position to whichever position shows the most of the bubble,
        // with tiebreaks going in the order: top > start > close > far.
        const mostOverlap = Math.max(topPositionOverlap, startPositionOverlap, closerPositionOverlap, fartherPositionOverlap);
     ...
     */
    getOverlap(relativeMin, viewMetrics) {
        ...
        return Math.max(0, Math.min(1, (overlapWidth * overlapHeight) / (this.size.width * this.size.height)));
    }
    /**
     ...
     *     bubble.
     */
    getOptimalRelativeLeft(viewMetrics) {
        ...
        if (this.size.width > viewMetrics.width)
            return relativeLeft;
     ...
     *     bubble.
     */
    getOptimalRelativeTop(viewMetrics) {
        ...
        if (this.size.height > viewMetrics.height)
            return relativeTop;
     ...
     * coordinates.
     */
    getWorkspaceViewRect(viewMetrics) {
    ...
    renderTail() {
        ...
        if (relBubbleX === relAnchorX && relBubbleY === relAnchorY) {
            ...
            let thickness = (this.size.width + this.size.height) / _a.TAIL_THICKNESS;
            thickness = Math.min(thickness, this.size.width, this.size.height) / 4;
            // Back the tip of the tail off of the anchor.
            const backoffRatio = 1 - _a.ANCHOR_RADIUS / hypotenuse;
            ...
            const radians = math.toRadians(this.workspace.RTL ? -_a.TAIL_ANGLE : _a.TAIL_ANGLE);
            let swirlAngle = angle + radians;
            if (swirlAngle > Math.PI * 2) {
                swirlAngle -= Math.PI * 2;
            }
            const swirlRise = (Math.sin(swirlAngle) * hypotenuse) / _a.TAIL_BEND;
            const swirlRun = (Math.cos(swirlAngle) * hypotenuse) / _a.TAIL_BEND;
    ...
    getRelativeToSurfaceXY() {
        return new Coordinate(this.workspace.RTL
            ? -this.relativeLeft + this.anchor.x - this.size.width
            : this.anchor.x + this.relativeLeft, this.anchor.y + this.relativeTop);
    }
    /** @internal */
    getSvgRoot() {
        return this.svgRoot;
