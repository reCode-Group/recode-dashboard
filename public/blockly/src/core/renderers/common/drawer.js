...
export class Drawer {
    ...
    positionInlineInputConnection_(input) {
        ...
        if (input.connectionModel) {
            ...
            input.connectionModel.setOffsetInBlock(connX, yPos + input.connectionOffsetY);
        }
    }
    /**
    ...
    positionOutputConnection_() {
        if (this.info_.outputConnection) {
            ...
            const connX = this.info_.RTL ? -x : x;
            this.block_.outputConnection.setOffsetInBlock(connX, this.info_.outputConnection.connectionOffsetY);
        }
    }
    /**
     ...
     */
    getExpressionConnectionHighlightPath(connection) {
        let connPath = '';
     ...
     */
    getStatementConnectionHighlightPath(connection) {
        if (!isNotch(connection.shape)) {
            throw new Error('Statement connections should have notch shapes');
        }
        const xLen = this.constants_.NOTCH_OFFSET_LEFT - this.constants_.CORNER_RADIUS;
        return (svgPaths.moveBy(-xLen, 0) +
