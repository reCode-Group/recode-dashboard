...
export class RenderInfo {
    ...
    populateTopRow_() {
        ...
        if (hasHat) {
            ...
            this.topRow.connection = new PreviousConnection(this.constants_, this.block_.previousConnection);
    ...
    populateBottomRow_() {
        ...
        if (this.bottomRow.hasNextConnection) {
            this.bottomRow.connection = new NextConnection(this.constants_, this.block_.nextConnection);
    ...
    addInput_(input, activeRow) {
        ...
        if (this.isInline && input instanceof ValueInput) {
            ...
            activeRow.minHeight = Math.max(activeRow.minHeight, input.getSourceBlock() && input.getSourceBlock().isShadow()
    ...
    shouldStartNewRow_(currInput, prevInput) {
        ...
        if (currInput instanceof StatementInput ||
        ...
        if (currInput instanceof ValueInput ||
    ...
    addElemSpacing_() {
        for (let i = 0, row; (row = this.rows[i]); i++) {
            ...
            if (row.startsWithElemSpacer()) {
                ...
                row.elements.push(new InRowSpacer(this.constants_, this.getInRowSpacing_(null, oldElems[0])));
            }
            if (!oldElems.length) {
                continue;
            }
            ...
            if (row.endsWithElemSpacer()) {
                ...
                row.elements.push(new InRowSpacer(this.constants_, this.getInRowSpacing_(oldElems[oldElems.length - 1], null)));
            }
        }
    }
    /**
    ...
    alignStatementRow_(row) {
        ...
        row.widthWithConnectedBlocks = Math.max(row.width, this.statementEdge + row.connectedBlockWidths);
    }
    ...
    finalize_() {
        ...
        if (this.outputConnection && this.block_.nextConnection) {
            ...
            if (target) {
                ...
                widestRowWithConnectedBlocks = Math.max(widestRowWithConnectedBlocks, target.getHeightWidth().width);
            }
        }
