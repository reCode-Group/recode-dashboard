...
export class RenderInfo extends BaseRenderInfo {
    ...
    getSpacerRowHeight_(prev, next) {
        ...
        if (Types.isBottomRow(next)) {
            if (!this.outputConnection) {
                const topHeight = Math.max(this.topRow.minHeight, Math.max(this.constants_.NOTCH_HEIGHT, this.constants_.CORNER_RADIUS)) - this.constants_.CORNER_RADIUS;
                return topHeight;
            }
    ...
    addInput_(input, activeRow) {
        ...
        if ((input instanceof DummyInput || input instanceof EndRowInput) &&
     ...
     */
    adjustXPosition_() {
        const notchTotalWidth = this.constants_.NOTCH_OFFSET_LEFT + this.constants_.NOTCH_WIDTH;
    ...
    finalizeOutputConnection_() {
        ...
        if (!('isDynamic' in outputConnectionShape && outputConnectionShape.isDynamic)) {
            return;
        }
        let yCursor = 0;
    ...
    finalizeHorizontalAlignment_() {
        ...
        for (let i = 0; i < this.rows.length; i++) {
            ...
            const minBlockWidth = this.constants_.MIN_BLOCK_WIDTH + this.outputConnection.width * 2;
