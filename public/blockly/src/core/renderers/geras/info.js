...
export class RenderInfo extends BaseRenderInfo {
    ...
    addInput_(input, activeRow) {
        ...
        if (this.isInline && input instanceof ValueInput) {
            ...
            activeRow.minHeight = Math.max(activeRow.minHeight, this.constants_.DUMMY_INPUT_MIN_HEIGHT);
    ...
    alignRowElements_() {
        ...
        for (let i = 0, row; (row = this.rows[i]); i++) {
            if (row.hasStatement) {
                ...
                const desiredWidth = Math.max(prevRightEdge, rowNextRightEdges.get(row));
    ...
    finalize_() {
        ...
        if (this.outputConnection &&
            ...
            if (target) {
                ...
                widestRowWithConnectedBlocks = Math.max(widestRowWithConnectedBlocks, target.getHeightWidth().width - this.constants_.DARK_PATH_OFFSET);
            }
        }
