...
export class FieldInput extends Field {
     ...
     */
    doValueInvalid_(_invalidValue, fireChangeEvent = true) {
        if (this.isBeingEdited_) {
            ...
            if (this.sourceBlock_ &&
                ...
                fireChangeEvent) {
                eventUtils.fire(new (eventUtils.get(EventType.BLOCK_CHANGE))(this.sourceBlock_, 'field', this.name || null, oldValue, this.value_));
            }
        }
    }
    /**
    ...
    applyColour() {
        ...
        if (!this.isFullBlockField() && this.borderRect_) {
            ...
            // impacts.
            block.pathObject.svgPath.setAttribute('fill', this.getConstants().FIELD_BORDER_RECT_COLOUR);
        }
    }
    /**
     ...
     *     Defaults to true.
     */
    showEditor_(_e, quietInput = false, manageEphemeralFocus = true) {
     ...
     */
    showInlineEditor(quietInput, manageEphemeralFocus) {
        const block = this.getSourceBlock();
        if (!block) {
            throw new UnattachedFieldError();
        }
        WidgetDiv.show(this, block.RTL, this.widgetDispose_.bind(this), this.workspace_, manageEphemeralFocus);
    ...
    widgetDispose_() {
        ...
        if (this.sourceBlock_ &&
            ...
            eventUtils.fire(new (eventUtils.get(EventType.BLOCK_CHANGE))(this.sourceBlock_, 'field', this.name || null, this.valueWhenEditorWasOpened_, this.value_));
            this.valueWhenEditorWasOpened_ = null;
        }
    ...
    bindInputEvents_(htmlInput) {
        ...
        this.onKeyDownWrapper = browserEvents.conditionalBind(htmlInput, 'keydown', this, this.onHtmlInputKeyDown_);
        // Resize after every input change.
        this.onKeyInputWrapper = browserEvents.conditionalBind(htmlInput, 'input', this, this.onHtmlInputChange);
    }
    ...
    onHtmlInputKeyDown_(e) {
        if (e.key === 'Enter') {
            ...
            const isValidDestination = (node) => (node instanceof FieldInput ||
    ...
    onHtmlInputChange(_e) {
        ...
        if (this.sourceBlock_ &&
            ...
            eventUtils.fire(new (eventUtils.get(EventType.BLOCK_FIELD_INTERMEDIATE_CHANGE))(this.sourceBlock_, this.name || null, oldValue, this.value_));
        }
    }
    /**
     ...
     */
    setEditorValue_(newValue, fireChangeEvent = true) {
        this.isDirty_ = true;
        if (this.isBeingEdited_) {
    ...
    repositionForWindowResize() {
        ...
        if (!(block instanceof BlockSvg))
            return false;
        const bumped = bumpObjects.bumpIntoBounds(this.workspace_, this.workspace_.getMetricsManager().getViewMetrics(true), block);
        if (!bumped)
            this.resizeEditor_();
        return true;
    }
