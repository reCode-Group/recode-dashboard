 ...
 * @param button
 */
function stringButtonClickHandler(button) {
    Variables.createVariableButtonHandler(button.getTargetWorkspace(), undefined, 'String');
}
...
function numberButtonClickHandler(button) {
    Variables.createVariableButtonHandler(button.getTargetWorkspace(), undefined, 'Number');
}
// eslint-disable-next-line camelcase
 ...
 * @param button
 */
function colourButtonClickHandler(button) {
    Variables.createVariableButtonHandler(button.getTargetWorkspace(), undefined, 'Colour');
}
// eslint-disable-next-line camelcase
...
export function flyoutCategory(workspace, useXml = true) {
    if (!Blocks['variables_set_dynamic'] && !Blocks['variables_get_dynamic']) {
        console.warn('There are no dynamic variable blocks, but there is a dynamic variable category.');
    }
    if (useXml) {
        deprecation.warn('The XML return value of Blockly.VariablesDynamic.flyoutCategory()', 'v12', 'v13', 'the same method, but handle a return type of FlyoutItemInfo[] (JSON) instead.');
        return xmlFlyoutCategory(workspace);
    }
    workspace.registerButtonCallback('CREATE_VARIABLE_STRING', stringButtonClickHandler);
    workspace.registerButtonCallback('CREATE_VARIABLE_NUMBER', numberButtonClickHandler);
    workspace.registerButtonCallback('CREATE_VARIABLE_COLOUR', colourButtonClickHandler);
    return [
        {
            'kind': 'button',
        ...
        ...Variables.jsonFlyoutCategoryBlocks(workspace, workspace.getVariableMap().getAllVariables(), false, 'variables_get_dynamic', 'variables_set_dynamic'),
    ];
}
/**
 * Construct the elements (blocks and button) required by the flyout for the
