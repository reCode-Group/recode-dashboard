 ...
 */
export function internalFlyoutCategory(workspace) {
 ...
 */
export function flyoutCategory(workspace, useXml = true) {
    if (!Blocks['variables_set'] && !Blocks['variables_get']) {
        console.warn('There are no variable blocks, but there is a variable category.');
    }
    if (useXml) {
        deprecation.warn('The XML return value of Blockly.Variables.flyoutCategory()', 'v12', 'v13', 'the same method, but handle a return type of FlyoutItemInfo[] (JSON) instead.');
    ...
    return [
        ...
        ...jsonFlyoutCategoryBlocks(workspace, workspace.getVariableMap().getVariablesOfType(''), true),
    ];
}
/**
 ...
 */
function generateVariableFieldJson(variable) {
    return {
        'VAR': {
 ...
 */
export function jsonFlyoutCategoryBlocks(workspace, variables, includeChangeBlocks, getterType = 'variables_get', setterType = 'variables_set') {
    includeChangeBlocks && (includeChangeBlocks = Blocks['math_change']);
    const blocks = [];
    ...
    if (Blocks[getterType]) {
        ...
        blocks.push(...variables.sort(compareByName).map((variable) => {
            return {
                'kind': 'block',
                'type': getterType,
                'gap': 8,
 ...
 */
function xmlFlyoutCategory(workspace) {
...
export function flyoutCategoryBlocks(workspace) {
    ...
    if (variableModelList.length > 0) {
        ...
        if (Blocks['math_change']) {
            ...
            block.appendChild(generateVariableFieldDom(mostRecentVariable));
            const value = utilsXml.textToDom('<value name="DELTA">' +
 ...
 */
function generateUniqueNameInternal(workspace) {
    return generateUniqueNameFromOptions(VAR_LETTER_OPTIONS.charAt(0), workspace
        .getVariableMap()
        .getAllVariables()
        .map((v) => v.getName()));
}
 ...
 */
export function createVariableButtonHandler(workspace, opt_callback, opt_type) {
    const type = opt_type || '';
...
export function renameVariable(workspace, variable, opt_callback) {
    // This function needs to be named so it can be called recursively.
    function promptAndCheckWithAlert(defaultName) {
        const promptText = Msg['RENAME_VARIABLE_TITLE'].replace('%1', variable.getName());
        promptName(promptText, defaultName, function (newName) {
            ...
            const existing = nameUsedWithOtherType(newName, variable.getType(), workspace);
            const procedure = nameUsedWithConflictingParam(variable.getName(), newName, workspace);
 ...
 *     none was found.
 */
function nameUsedWithOtherType(name, type, workspace) {
 ...
 * @internal
 */
export function nameUsedWithConflictingParam(oldName, newName, workspace) {
 ...
 */
function checkForConflictingParamWithProcedureModels(oldName, newName, workspace) {
 ...
 * null if one does not exist.
 */
function checkForConflictingParamWithLegacyProcedures(oldName, newName, workspace) {
    oldName = oldName.toLowerCase();
    newName = newName.toLowerCase();
    const blocks = workspace.getAllBlocks(false);
 ...
 *     combination.
 */
function createVariable(workspace, id, opt_name, opt_type) {
    ...
    if (potentialVariableMap) {
        variable = potentialVariableMap.createVariable(opt_name, opt_type, id ?? undefined);
    }
    else {
 ...
 * @internal
 */
export function compareByName(var1, var2) {
    return var1
        .getName()
 ...
 */
export function getVariableUsesById(workspace, id) {
    const uses = [];
    const blocks = workspace.getAllBlocks(false);
 ...
 */
export function deleteVariable(workspace, variable, triggeringBlock) {
