...
export class Scrollbar {
    ...
    constructor(workspace, horizontal, opt_pair, opt_class, opt_margin) {
        ...
        let className = 'blocklyScrollbar' + (this.horizontal ? 'Horizontal' : 'Vertical');
        if (opt_class) {
            className += ' ' + opt_class;
        }
        ...
        */
        ...
        this.svgBackground = dom.createSvgElement(Svg.RECT, { 'class': 'blocklyScrollbarBackground' }, group);
        const radius = Math.floor((Scrollbar.scrollbarThickness - 5) / 2);
        this.svgHandle = dom.createSvgElement(Svg.RECT, { 'class': 'blocklyScrollbarHandle', 'rx': radius, 'ry': radius }, group);
        this.workspace
            .getThemeManager()
        ...
        this.onMouseDownBarWrapper_ = browserEvents.conditionalBind(this.svgBackground, 'pointerdown', this, this.onMouseDownBar);
        this.onMouseDownHandleWrapper_ = browserEvents.conditionalBind(this.svgHandle, 'pointerdown', this, this.onMouseDownHandle);
    }
    /**
     ...
     */
    requiresViewResize(hostMetrics) {
        if (!this.oldHostMetrics) {
            return true;
        }
    ...
    resizeViewHorizontal(hostMetrics) {
        ...
        // workspace.
        const yCoordinate = hostMetrics.absoluteTop +
    ...
    resizeContentHorizontal(hostMetrics) {
        ...
        // Resize the handle.
        let handleLength = (this.scrollbarLength * hostMetrics.viewWidth) / hostMetrics.scrollWidth;
    ...
    onMouseDownBar(e) {
        ...
        const mouseXY = browserEvents.mouseToSvg(e, this.workspace.getParentSvg(), this.workspace.getInverseScreenCTM());
    ...
    onMouseDownHandle(e) {
        ...
        this.onMouseUpWrapper_ = browserEvents.conditionalBind(document, 'pointerup', this, this.onMouseUpHandle);
        this.onMouseMoveWrapper_ = browserEvents.conditionalBind(document, 'pointermove', this, this.onMouseMoveHandle);
        e.stopPropagation();
        e.preventDefault();
    }
    /**
     ...
     */
    static metricsAreEquivalent(first, second) {
 ...
 */
Scrollbar.scrollbarThickness = Touch.TOUCH_ENABLED ? 20 : 12;
/**
 ...
 * @internal
 */
Scrollbar.DEFAULT_SCROLLBAR_MARGIN = 0.8;
