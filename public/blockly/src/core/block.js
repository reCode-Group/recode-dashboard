...
export class Block {
    ...
    unplugFromRow(opt_healStack) {
        ...
        if (this.workspace.connectionChecker.canConnect(childConnection, parentConnection, false)) {
    ...
    unplugFromStack(opt_healStack) {
        ...
        if (opt_healStack && nextBlock && !nextBlock.isShadow()) {
            ...
            if (previousTarget &&
                this.workspace.connectionChecker.canConnect(previousTarget, nextTarget, false)) {
    ...
    isDuplicatable() {
        ...
        return this.workspace.isCapacityAvailable(common.getBlockTypeCounts(this, true));
    }
    /**
    ...
    *getFields() {
    ...
    setPreviousStatement(newBoolean, opt_check) {
        if (newBoolean) {
            ...
            if (!this.previousConnection) {
                this.previousConnection = this.makeConnection_(ConnectionType.PREVIOUS_STATEMENT);
            }
    ...
    setInputsInline(newBoolean) {
        if (this.inputsInline !== newBoolean) {
            eventUtils.fire(new (eventUtils.get(EventType.BLOCK_CHANGE))(this, 'inline', null, this.inputsInline, newBoolean));
    ...
    setDisabledReason(disabled, reason) {
        ...
        if (this.disabledReasons.has(reason) !== disabled) {
            ...
            const blockChangeEvent = new (eventUtils.get(EventType.BLOCK_CHANGE))(this, 'disabled', 
            /* name= */ null, 
            /* oldValue= */ !disabled, 
            /* newValue= */ disabled);
    ...
    toString(opt_maxLength, opt_emptyToken) {
        ...
        let prev = '';
        let text = tokens.reduce((acc, curr) => {
    ...
    toTokens(emptyToken = '?') {
        ...
        function shouldAddParentheses(connection) {
            ...
            return (!!checks && (checks.includes('Boolean') || checks.includes('Number')));
        }
        for (const input of this.inputList) {
     ...
     */
    appendInputFromRegistry(type, name) {
        const inputConstructor = registry.getClass(registry.Type.INPUT, type, false);
        if (!inputConstructor)
            return null;
    ...
    jsonInit(json) {
        ...
        while (json['message' + i] !== undefined) {
            this.interpolate(json['message' + i], json['args' + i] || [], 
            // Backwards compatibility: lastDummyAlign aliases implicitAlign.
            json['implicitAlign' + i] || json['lastDummyAlign' + i], warningPrefix);
            i++;
        }
     ...
     */
    jsonInitColour(json, warningPrefix) {
        if ('colour' in json) {
     ...
     * @param warningPrefix Warning prefix string identifying block.
     */
    jsonInitStyle(json, warningPrefix) {
     ...
     * @param warningPrefix Warning prefix string identifying block.
     */
    interpolate(message, args, implicitAlign, warningPrefix) {
     ...
     */
    validateTokens(tokens, argsCount) {
     ...
     */
    interpolateArguments(tokens, args, implicitAlign) {
        const elements = [];
        for (let i = 0; i < tokens.length; i++) {
        ...
        if (length &&
            !this.isInputKeyword(elements[length - 1]['type'])) {
            const dummyInput = { 'type': 'input_dummy' };
            if (implicitAlign) {
                dummyInput['align'] = implicitAlign;
            }
     ...
     */
    inputFromJson(element, warningPrefix) {
    ...
    setCommentText(text) {
        ...
        if (text !== null) {
            ...
            if (!comment) {
                const commentConstructor = registry.getClass(registry.Type.ICON, IconType.COMMENT.toString(), false);
                if (!commentConstructor) {
                    throw new Error('No comment icon class is registered, so a comment cannot be set');
                }
                const icon = new commentConstructor(this);
                if (!isCommentIcon(icon)) {
                    throw new Error('The class registered as a comment icon does not conform to the ' +
        ...
        eventUtils.fire(new (eventUtils.get(EventType.BLOCK_CHANGE))(this, 'comment', null, oldText, text));
    }
    /**
    ...
    addIcon(icon) {
        if (this.hasIcon(icon.getType()))
            throw new DuplicateIconType(icon);
    ...
    getIcon(type) {
        if (type instanceof IconType) {
            return this.icons.find((icon) => icon.getType().equals(type));
        }
        else {
            return this.icons.find((icon) => icon.getType().toString() === type);
        }
    }
    ...
    moveBy(dx, dy, reason) {
        ...
        const event = new (eventUtils.get(EventType.BLOCK_MOVE))(this);
        if (reason)
