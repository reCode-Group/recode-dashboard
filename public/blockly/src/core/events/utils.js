...
import { isBlockChange, isBlockCreate, isBlockMove, isBubbleOpen, isClick, isViewportChange, } from './predicates.js';
...
function enqueueEvent(event) {
    if (isBlockChange(event) && event.element === 'mutation') {
        ...
        for (i = FIRE_QUEUE.length; i > 0; i--) {
            ...
            if (otherEvent.group !== event.group ||
    ...
    for (const event of queue) {
        ...
        if (isBlockMove(event) &&
            ...
            if (lastEvent.reason || event.reason) {
                lastEvent.reason = Array.from(new Set((lastEvent.reason ?? []).concat(event.reason ?? [])));
            }
        }
        else if (isBlockChange(event) &&
 ...
 */
function setGroupInternal(state) {
...
export function disableOrphans(event) {
    if (isBlockMove(event) || isBlockCreate(event)) {
        ...
        const eventWorkspace = common.getWorkspaceById(blockEvent.workspaceId);
