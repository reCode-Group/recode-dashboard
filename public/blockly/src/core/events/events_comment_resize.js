...
export class CommentResize extends CommentBase {
    ...
    constructor(opt_comment) {
        super(opt_comment);
        this.type = EventType.COMMENT_RESIZE;
        if (!opt_comment) {
    ...
    recordCurrentSizeAsNewSize() {
        if (this.newSize) {
            throw Error('Tried to record the new size of a comment on the ' +
                'same event twice.');
        }
        ...
        if (!comment) {
            throw new Error('The comment associated with the comment resize event ' +
                'could not be found');
        }
    ...
    toJson() {
        ...
        if (!this.oldSize) {
            throw new Error('The old comment size is undefined. Either pass a comment to ' +
                'the constructor, or call fromJson');
        }
        if (!this.newSize) {
            throw new Error('The new comment size is undefined. Either call ' +
    ...
    static fromJson(json, workspace, event) {
        const newEvent = super.fromJson(json, workspace, event ?? new CommentResize());
    ...
    run(forward) {
        ...
        if (!size) {
            throw new Error('Either oldSize or newSize is undefined. ' +
...
//# sourceMappingURL=events_comment_resize.js.map