...
Extensions.register('logic_op_tooltip', Extensions.buildTooltipForDropdown('OP', TOOLTIPS_BY_OP));
/**
...
const CONTROLS_IF_MUTATOR_MIXIN = {
    ...
    updateShape_: function () {
        ...
        for (let i = 1; i <= this.elseifCount_; i++) {
            ...
            this.appendStatementInput('DO' + i).appendField(Msg['CONTROLS_IF_MSG_THEN']);
        }
        if (this.elseCount_) {
            this.appendStatementInput('ELSE').appendField(Msg['CONTROLS_IF_MSG_ELSE']);
        }
    },
    /**
...
Extensions.registerMutator('controls_if_mutator', CONTROLS_IF_MUTATOR_MIXIN, null, // TODO(#6920)
...
const LOGIC_COMPARE_ONCHANGE_MIXIN = {
    ...
    onchange: function (e) {
        ...
        if (blockA &&
            blockB &&
            !this.workspace.connectionChecker.doTypeChecks(blockA.outputConnection, blockB.outputConnection)) {
    ...
    onchange: function (e) {
        ...
        if ((blockA || blockB) && parentConnection) {
            for (let i = 0; i < 2; i++) {
                ...
                if (block &&
                    !block.workspace.connectionChecker.doTypeChecks(block.outputConnection, parentConnection)) {
