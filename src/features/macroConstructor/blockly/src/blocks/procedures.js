...
const PROCEDURE_DEF_COMMON = {
    ...
    setStatements_: function (hasStatements) {
        ...
        if (hasStatements) {
            this.appendStatementInput('STACK').appendField(Msg['PROCEDURES_DEFNORETURN_DO']);
    ...
    domToMutation: function (xmlElement) {
        ...
        for (let i = 0, childNode; (childNode = xmlElement.childNodes[i]); i++) {
            if (childNode.nodeName.toLowerCase() === 'arg') {
                ...
                const varId = childElement.getAttribute('varid') ||
                    childElement.getAttribute('varId');
                this.arguments_.push(varName);
                const variable = Variables.getOrCreateVariablePackage(this.workspace, varId, varName, '');
                if (variable !== null) {
                    this.argumentVarModels_.push(variable);
                }
                else {
                    console.log(`Failed to create a variable named "${varName}", ignoring.`);
                }
            }
        }
        this.updateParams_();
    ...
    loadExtraState: function (state) {
        ...
        if (state['params']) {
            for (let i = 0; i < state['params'].length; i++) {
                ...
                const variable = Variables.getOrCreateVariablePackage(this.workspace, param['id'], param['name'], '');
...
const PROCEDURES_MUTATORCONTAINER = {
    ...
    init: function () {
        ...
        this.appendDummyInput('STATEMENT_INPUT')
            ...
            .appendField(fieldRegistry.fromJson({
            type: 'field_checkbox',
            checked: true,
        }), 'STATEMENTS');
        this.setStyle('procedure_blocks');
...
const PROCEDURES_MUTATORARGUMENT = {
     ...
     */
    init: function () {
        const field = new ProcedureArgumentField(Procedures.DEFAULT_ARG, this.validator_);
        this.appendDummyInput()
 ...
 */
const DISABLED_PROCEDURE_DEFINITION_DISABLED_REASON = 'DISABLED_PROCEDURE_DEFINITION';
/**
...
const PROCEDURE_CALL_COMMON = {
    ...
    setProcedureParameters_: function (paramNames, paramIds) {
        ...
        const defBlock = Procedures.getDefinition(this.getProcedureCall(), this.workspace);
        ...
        this.argumentVarModels_ = [];
        for (let i = 0; i < this.arguments_.length; i++) {
            const variable = Variables.getOrCreateVariablePackage(this.workspace, null, this.arguments_[i], '');
            this.argumentVarModels_.push(variable);
        }
        this.updateShape_();
        this.quarkIds_ = paramIds;
    ...
    onchange: function (event) {
        ...
        if (event.type === Events.BLOCK_CREATE &&
            ...
            if (!def) {
                ...
                this.setWarningText(Msg['PROCEDURES_CALL_DISABLED_DEF_WARNING'].replace('%1', name));
            }
        }
