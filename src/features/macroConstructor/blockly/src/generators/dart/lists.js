...
export function lists_length(block, generator) {
    ...
    const list = generator.valueToCode(block, 'VALUE', Order.UNARY_POSTFIX) || '[]';
...
`);
                ...
                const at = generator.getAdjusted(block, 'AT', 1);
                // We need to create a procedure to avoid reevaluating values.
                const functionName = generator.provideFunction_('lists_remove_from_end', `
dynamic ${generator.FUNCTION_NAME_PLACEHOLDER_}(List my_list, num x) {
  x = my_list.length - x;
...
export function lists_getSublist(block, generator) {
    // Get sublist.
    const list = generator.valueToCode(block, 'LIST', Order.UNARY_POSTFIX) || '[]';
