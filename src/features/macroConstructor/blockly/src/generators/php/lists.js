...
export function lists_isEmpty(block, generator) {
    ...
    const argument0 = generator.valueToCode(block, 'VALUE', Order.FUNCTION_CALL) || 'array()';
...
export function lists_getIndex(block, generator) {
    ...
    switch (where) {
        case 'FIRST':
            if (mode === 'GET') {
                const list = generator.valueToCode(block, 'VALUE', Order.MEMBER) || 'array()';
                const code = list + '[0]';
                return [code, Order.MEMBER];
            }
            else if (mode === 'GET_REMOVE') {
                const list = generator.valueToCode(block, 'VALUE', Order.NONE) || 'array()';
        ...
        case 'FROM_END':
            if (mode === 'GET') {
                ...
                const code = 'array_splice(' + list + ', count(' + list + ') - ' + at + ', 1)[0]';
                if (mode === 'GET_REMOVE') {
                    return [code, Order.FUNCTION_CALL];
                }
                else if (mode === 'REMOVE') {
        ...
        case 'RANDOM': {
            const list = generator.valueToCode(block, 'VALUE', Order.NONE) || 'array()';
            if (mode === 'GET') {
                const functionName = generator.provideFunction_('lists_get_random_item', `
...
`);
                ...
                const functionName = generator.provideFunction_('lists_get_remove_random_item', `
function ${generator.FUNCTION_NAME_PLACEHOLDER_}(&$list) {
...
`);
                ...
                const functionName = generator.provideFunction_('lists_remove_random_item', `
function ${generator.FUNCTION_NAME_PLACEHOLDER_}(&$list) {
...
export function lists_setIndex(block, generator) {
    ...
    function cacheList() {
        ...
        const listVar = generator.nameDB_.getDistinctName('tmp_list', NameType.VARIABLE);
    ...
    switch (where) {
        case 'FIRST':
            if (mode === 'SET') {
                const list = generator.valueToCode(block, 'LIST', Order.MEMBER) || 'array()';
                return list + '[0] = ' + value + ';\n';
            }
            else if (mode === 'INSERT') {
        ...
        case 'LAST': {
            const list = generator.valueToCode(block, 'LIST', Order.NONE) || 'array()';
            if (mode === 'SET') {
                const functionName = generator.provideFunction_('lists_set_last_item', `
...
`);
        ...
        case 'FROM_END': {
            ...
            if (mode === 'SET') {
                const functionName = generator.provideFunction_('lists_set_from_end', `
...
`);
                ...
                const functionName = generator.provideFunction_('lists_insert_from_end', `
function ${generator.FUNCTION_NAME_PLACEHOLDER_}(&$list, $at, $value) {
...
`);
        ...
        case 'RANDOM':
            ...
            const xVar = generator.nameDB_.getDistinctName('tmp_x', NameType.VARIABLE);
...
export function lists_sort(block, generator) {
    ...
    const listCode = generator.valueToCode(block, 'LIST', Order.NONE) || 'array()';
...
`);
    const sortCode = functionName + '(' + listCode + ', "' + type + '", ' + direction + ')';
