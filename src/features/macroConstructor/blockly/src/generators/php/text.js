...
export function text_join(block, generator) {
    ...
    if (joinBlock.itemCount_ === 0) {
        ...
        const element0 = generator.valueToCode(block, 'ADD0', Order.STRING_CONCAT) || "''";
        const element1 = generator.valueToCode(block, 'ADD1', Order.STRING_CONCAT) || "''";
...
export function text_indexOf(block, generator) {
    ...
    const operator = block.getFieldValue('END') === 'FIRST' ? 'strpos' : 'strrpos';
    ...
    const functionName = generator.provideFunction_(block.getFieldValue('END') === 'FIRST'
        ? 'text_indexOf'
        : 'text_lastIndexOf', `
