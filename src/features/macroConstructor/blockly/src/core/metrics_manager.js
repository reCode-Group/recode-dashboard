...
export class MetricsManager {
    ...
    getFlyoutMetrics(opt_own) {
        const flyoutDimensions = this.getDimensionsPx_(this.workspace_.getFlyout(opt_own));
        return {
    ...
    getViewMetrics(opt_getWorkspaceCoordinates) {
        ...
        const toolboxPosition = respectToolbox
            ? toolboxMetrics.position
            : flyoutMetrics.position;
        const horizToolbox = toolboxPosition === toolboxUtils.Position.TOP ||
            toolboxPosition === toolboxUtils.Position.BOTTOM;
        const vertToolbox = toolboxPosition === toolboxUtils.Position.LEFT ||
    ...
    getPaddedContent_(viewMetrics, contentMetrics) {
        ...
        const top = Math.min(contentMetrics.top - halfHeight, contentBottom - viewHeight);
        const left = Math.min(contentMetrics.left - halfWidth, contentRight - viewWidth);
        const bottom = Math.max(contentBottom + halfHeight, contentMetrics.top + viewHeight);
        const right = Math.max(contentRight + halfWidth, contentMetrics.left + viewWidth);
...
//# sourceMappingURL=metrics_manager.js.map