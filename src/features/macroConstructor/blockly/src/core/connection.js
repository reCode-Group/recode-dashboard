...
export class Connection {
    ...
    connect_(childConnection) {
        ...
        if (eventUtils.isEnabled()) {
            event = new (eventUtils.get(EventType.BLOCK_MOVE))(childBlock);
        ...
        if (orphan) {
            const orphanConnection = this.type === INPUT
                ? orphan.outputConnection
                : orphan.previousConnection;
            if (!orphanConnection)
                return;
            const connection = Connection.getConnectionForOrphanedConnection(childBlock, orphanConnection);
            if (connection) {
     ...
     * @internal
     */
    onFailedConnect(_superiorConnection) { }
    // NOP
    /**
    ...
    disconnectInternal(setParent = true) {
        ...
        let event;
        if (eventUtils.isEnabled()) {
            event = new (eventUtils.get(EventType.BLOCK_MOVE))(childConnection.getSourceBlock());
    ...
    getShadowDom(returnCurrent) {
        return returnCurrent && this.targetBlock().isShadow()
            ? Xml.blockToDom(this.targetBlock())
            : this.shadowDom;
    }
    /**
     * Changes the connection's shadow block.
    ...
    getShadowState(returnCurrent) {
        if (returnCurrent && this.targetBlock() && this.targetBlock().isShadow()) {
            return blocks.save(this.targetBlock());
        }
     ...
     *     properties.
     */
    applyShadowState({ shadowDom, shadowState, }) {
     ...
     */
    setShadowStateInternal({ shadowDom = null, shadowState = null, } = {}) {
     ...
     */
    createShadowBlock(attemptToConnect) {
        const parentBlock = this.getSourceBlock();
        ...
        if (shadowDom) {
            ...
            if (attemptToConnect) {
                if (this.type === ConnectionType.INPUT_VALUE) {
                    ...
                    throw new Error('Cannot connect a shadow block to a previous/output connection');
                }
            }
            return blockShadow;
        }
     ...
     */
    serializeShadow(shadow) {
        if (!shadow) {
            return;
        }
        this.shadowDom = Xml.blockToDom(shadow);
    ...
    static getConnectionForOrphanedConnection(startBlock, orphanConnection) {
        if (orphanConnection.type === ConnectionType.OUTPUT_VALUE) {
            return getConnectionForOrphanedOutput(startBlock, orphanConnection.getSourceBlock());
        }
 ...
 */
function getSingleConnection(block, orphanBlock) {
