...
export class HorizontalFlyout extends Flyout {
    ...
    setMetrics_(xyRatio) {
        ...
        if (typeof xyRatio.x === 'number') {
            this.workspace_.scrollX = -(scrollMetrics.left +
    ...
    reflowInternal_() {
        ...
        let flyoutHeight = this.getContents().reduce((maxHeightSoFar, item) => {
            return Math.max(maxHeightSoFar, item.getElement().getBoundingRectangle().getHeight());
        }, 0);
        ...
        if (this.getHeight() !== flyoutHeight) {
            ...
            if (!this.targetWorkspace.scrollbar &&
                ...
                this.targetWorkspace.translate(this.targetWorkspace.scrollX, this.targetWorkspace.scrollY + flyoutHeight);
            }
...
registry.register(registry.Type.FLYOUTS_HORIZONTAL_TOOLBOX, registry.DEFAULT, HorizontalFlyout);
//# sourceMappingURL=flyout_horizontal.js.map