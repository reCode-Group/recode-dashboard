 ...
 */
export function show(menuOpenEvent, options, rtl, workspace, location) {
 ...
 */
function populate_(options, rtl, menuOpenEvent, location) {
    ...
    for (let i = 0; i < options.length; i++) {
        ...
        if (option.enabled) {
            const actionHandler = function (p1, menuSelectEvent) {
                hide();
                requestAnimationFrame(() => {
                    setTimeout(() => {
                        ...
                        option.callback(option.scope, menuOpenEvent, menuSelectEvent, location);
                    }, 0);
                });
            };
 ...
 */
function position_(menu, rtl, location) {
    // Record windowSize and scrollOffset before adding menu.
    const viewportBBox = svgMath.getViewportBBox();
    // This one is just a point, but we'll pretend that it's a rect so we can use
    // some helper functions.
    const anchorBBox = new Rect(location.y + viewportBBox.top, location.y + viewportBBox.top, location.x + viewportBBox.left, location.x + viewportBBox.left);
    createWidget_(menu);
...
function createWidget_(menu) {
    ...
    browserEvents.conditionalBind(menuDom, 'contextmenu', null, haltPropagation);
 ...
 */
export function callbackFactory(block, state) {
    return () => {
        eventUtils.disable();
        let newBlock;
        try {
            if (state instanceof Element) {
                newBlock = Xml.domToBlockInternal(state, block.workspace);
            }
            else {
                newBlock = serializationBlocks.appendInternal(state, block.workspace);
            }
