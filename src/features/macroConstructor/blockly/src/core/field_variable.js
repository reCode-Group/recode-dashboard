...
export class FieldVariable extends FieldDropdown {
    ...
    initModel() {
        ...
        const variable = Variables.getOrCreateVariablePackage(block.workspace, null, this.defaultVariableName, this.defaultType);
    ...
    fromXml(fieldElement) {
        ...
        const variable = Variables.getOrCreateVariablePackage(block.workspace, id, variableName, variableType);
    ...
    saveState(doFullSerialization) {
        ...
        if (doFullSerialization) {
            ...
            state['type'] = this.variable.getType();
        }
        return state;
    }
    /**
    ...
    loadState(state) {
        ...
        const variable = Variables.getOrCreateVariablePackage(block.workspace, state['id'] || null, state['name'], state['type'] || '');
        this.setValue(variable.getId());
    }
    /**
    ...
    doClassValidation_(newValue) {
        ...
        if (!variable) {
            console.warn("Variable id doesn't point to a real variable! " + 'ID was ' + newId);
            return null;
        }
        // Type Checks.
    ...
    doValueUpdate_(newId) {
        ...
        this.variable = Variables.getVariable(block.workspace, newId);
     ...
     */
    setTypes(variableTypes = null, defaultType = '') {
        const name = this.getText();
        if (Array.isArray(variableTypes)) {
            if (variableTypes.length === 0) {
                // Throw an error if variableTypes is an empty list.
                throw Error(`'variableTypes' of field variable ${name} was an empty list. If you want to include all variable types, pass 'null' instead.`);
            }
            ...
            if (!isInArray) {
                throw Error("Invalid default type '" +
                    defaultType +
                    "' in " +
            ...
            throw Error(`'variableTypes' was not an array or null in the definition of FieldVariable ${name}`);
        }
        ...
        if (sourceBlock && !sourceBlock.isDeadOrDying()) {
            ...
            for (let i = 0; i < variableTypes.length; i++) {
                ...
                if (workspace.isFlyout) {
                    variableModelList = variableModelList.concat(workspace
...
//# sourceMappingURL=field_variable.js.map