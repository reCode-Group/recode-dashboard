 ...
 */
export class Trashcan extends DeleteArea {
    ...
    constructor(workspace) {
        ...
        if (this.workspace.horizontalLayout) {
            ...
            const HorizontalFlyout = registry.getClassFromOptions(registry.Type.FLYOUTS_HORIZONTAL_TOOLBOX, this.workspace.options, true);
            ...
            const VerticalFlyout = registry.getClassFromOptions(registry.Type.FLYOUTS_VERTICAL_TOOLBOX, this.workspace.options, true);
    ...
    createDom() {
        ...
        clip = dom.createSvgElement(Svg.CLIPPATH, { 'id': 'blocklyTrashBodyClipPath' + rnd }, this.svgGroup);
        dom.createSvgElement(Svg.RECT, { 'width': WIDTH, 'height': BODY_HEIGHT, 'y': LID_HEIGHT }, clip);
        const body = dom.createSvgElement(Svg.IMAGE, {
            'width': SPRITE.width,
            'x': -SPRITE_LEFT,
        ...
        body.setAttributeNS(dom.XLINK_NS, 'xlink:href', this.workspace.options.pathToMedia + SPRITE.url);
        clip = dom.createSvgElement(Svg.CLIPPATH, { 'id': 'blocklyTrashLidClipPath' + rnd }, this.svgGroup);
        dom.createSvgElement(Svg.RECT, { 'width': WIDTH, 'height': LID_HEIGHT }, clip);
        this.svgLid = dom.createSvgElement(Svg.IMAGE, {
            'width': SPRITE.width,
            'x': -SPRITE_LEFT,
            'height': SPRITE.height,
            'y': -SPRITE_TOP,
        ...
        this.svgLid.setAttributeNS(dom.XLINK_NS, 'xlink:href', this.workspace.options.pathToMedia + SPRITE.url);
        // bindEventWithChecks_ quashes events too aggressively. See:
        // https://groups.google.com/forum/#!topic/blockly/QF4yB9Wx00s
        // Using bindEventWithChecks_ for blocking mousedown causes issue in mobile.
        // See #4303
        browserEvents.bind(this.svgGroup, 'pointerdown', this, this.blockMouseDownWhenOpenable);
    ...
    position(metrics, savedPositions) {
        ...
        const startRect = uiPosition.getStartPositionRect(cornerPosition, new Size(WIDTH, height), MARGIN_HORIZONTAL, MARGIN_VERTICAL, metrics, this.workspace);
        ...
        const positionRect = uiPosition.bumpPositionRect(startRect, MARGIN_VERTICAL, bumpDirection, savedPositions);
    ...
    animateLid() {
        ...
        if (this.lidOpen > this.minOpenness && this.lidOpen < 1) {
            this.lidTask = setTimeout(this.animateLid.bind(this), ANIMATION_LENGTH / frames);
        }
    }
    /**
    ...
    setLidAngle(lidAngle) {
        const openAtRight = this.workspace.toolboxPosition === toolbox.Position.RIGHT ||
            (this.workspace.horizontalLayout && this.workspace.RTL);
        this.svgLid?.setAttribute('transform', 'rotate(' +
     ...
     */
    fireUiEvent(trashcanOpen) {
        const uiEvent = new (eventUtils.get(EventType.TRASHCAN_OPEN))(trashcanOpen, this.workspace.id);
