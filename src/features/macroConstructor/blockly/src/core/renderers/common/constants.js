 ...
 */
export function isDynamicShape(shape) {
    return shape.isDynamic;
}
/** Returns whether the shape is a puzzle tab or not. */
export function isPuzzleTab(shape) {
    return (shape.pathDown !== undefined &&
        shape.pathUp !== undefined);
}
/** Returns whether the shape is a notch or not. */
export function isNotch(shape) {
    return (shape.pathLeft !== undefined &&
        shape.pathRight !== undefined);
}
/**
...
export class ConstantProvider {
    ...
    setFontConstants_(theme) {
        ...
        const fontMetrics = dom.measureFontMetrics('Hg', this.FIELD_TEXT_FONTSIZE + 'pt', this.FIELD_TEXT_FONTWEIGHT, this.FIELD_TEXT_FONTFAMILY);
    ...
    createDom(svg, tagName, selector, injectionDivIfIsParent) {
        ...
        /*
            ...
            */
        const embossFilter = dom.createSvgElement(Svg.FILTER, { 'id': 'blocklyEmbossFilter' + this.randomIdentifier }, this.defs);
        dom.createSvgElement(Svg.FEGAUSSIANBLUR, { 'in': 'SourceAlpha', 'stdDeviation': 1, 'result': 'blur' }, embossFilter);
        const feSpecularLighting = dom.createSvgElement(Svg.FESPECULARLIGHTING, {
            'in': 'blur',
            'surfaceScale': 1,
        ...
        /*
            ...
            */
        const disabledPattern = dom.createSvgElement(Svg.PATTERN, {
        ...
        if (injectionDivIfIsParent) {
            ...
            injectionDivIfIsParent.style.setProperty('--blocklyEmbossFilter', `url(#${this.embossFilterId})`);
            injectionDivIfIsParent.style.setProperty('--blocklyDisabledPattern', `url(#${this.disabledPatternId})`);
            injectionDivIfIsParent.style.setProperty('--blocklyDebugFilter', `url(#${this.debugFilterId})`);
        }
    }
    /**
    ...
    createDebugFilter() {
        ...
        if (!this.debugFilter) {
            const debugFilter = dom.createSvgElement(Svg.FILTER, {
