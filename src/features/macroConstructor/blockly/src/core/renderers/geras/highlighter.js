...
export class Highlighter {
    ...
    drawTopCorner(row) {
        ...
        for (let i = 0, elem; (elem = row.elements[i]); i++) {
            if (Types.isLeftSquareCorner(elem)) {
                ...
                // move.
                this.steps_ += svgPaths.lineOnAxis('H', elem.xPos + elem.width - this.highlightOffset);
            }
        }
    ...
    drawJaggedEdge_(row) {
        if (this.info_.RTL) {
            const remainder = row.height - this.jaggedTeethPaths_.height - this.highlightOffset;
            this.steps_ +=
    ...
    drawValueInput(row) {
        ...
        if (this.RTL_) {
            ...
            this.steps_ +=
                svgPaths.moveTo(input.xPos + input.width - this.highlightOffset, row.yPos) +
        ...
        if (this.RTL_) {
            ...
            this.steps_ +=
                svgPaths.moveTo(input.xPos, row.yPos) +
                    ...
                    svgPaths.lineTo(row.width - input.xPos - this.insideCornerPaths_.width, 0);
        }
        else {
            this.steps_ +=
    ...
    drawRightSideRow(row) {
        ...
        if (this.RTL_) {
            ...
            if (row.height > this.highlightOffset) {
                this.steps_ += svgPaths.lineOnAxis('V', row.yPos + row.height - this.highlightOffset);
            }
        }
    }
    /**
    ...
    drawBottomRow(row) {
        ...
        if (this.RTL_) {
            this.steps_ += svgPaths.lineOnAxis('V', row.baseline - this.highlightOffset);
        }
        else {
            const cornerElem = this.info_.bottomRow.elements[0];
            if (Types.isLeftSquareCorner(cornerElem)) {
                this.steps_ += svgPaths.moveTo(row.xPos + this.highlightOffset, row.baseline - this.highlightOffset);
            }
    ...
    drawLeft() {
        ...
        if (outputConnection) {
            const tabBottom = outputConnection.connectionOffsetY + outputConnection.height;
        ...
        if (!this.RTL_) {
            ...
            if (Types.isLeftRoundedCorner(topRow.elements[0])) {
                ...
                this.steps_ += svgPaths.lineOnAxis('V', topRow.capline + this.highlightOffset);
            }
        }
    }
    /**
