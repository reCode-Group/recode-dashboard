...
export class HighlightConstantProvider {
    ...
    makeInsideCorner() {
        ...
        const pathTopRtl = svgPaths.moveBy(distance45outside, distance45outside) +
            svgPaths.arc('a', '0 0,0', radius, svgPaths.point(-distance45outside - offset, radius - distance45outside));
        const pathBottomRtl = svgPaths.arc('a', '0 0,0', radius + offset, svgPaths.point(radius + offset, radius + offset));
        const pathBottomLtr = svgPaths.moveBy(distance45outside, -distance45outside) +
            svgPaths.arc('a', '0 0,0', radius + offset, svgPaths.point(radius - distance45outside, distance45outside + offset));
        return {
            width: radius + offset,
            height: radius,
            pathTop(rtl) {
    ...
    makeOutsideCorner() {
        ...
        const topLeftCornerHighlightRtl = svgPaths.moveBy(topLeftStartX, topLeftStartY) +
            svgPaths.arc('a', '0 0,1', radius - offset, svgPaths.point(radius - topLeftStartX, -topLeftStartY + offset));
        /**
         * SVG path for drawing the highlight on the rounded top-left corner.
         */
        const topLeftCornerHighlightLtr = svgPaths.moveBy(offset, radius) +
            svgPaths.arc('a', '0 0,1', radius - offset, svgPaths.point(radius, -radius + offset));
        ...
        const bottomLeftPath = svgPaths.moveBy(bottomLeftStartX, bottomLeftStartY) +
            svgPaths.arc('a', '0 0,1', radius - offset, svgPaths.point(-bottomLeftStartX + offset, -bottomLeftStartY - radius));
        return {
            height: radius,
            topLeft(rtl) {
    ...
    makePuzzleTab() {
        ...
        const highlightLtrDown = svgPaths.moveBy(-5, height - 0.7) + svgPaths.lineTo(width * 0.46, -2.1);
        return {
            width,
            height,
            pathUp(rtl) {
     ...
     */
    makeJaggedTeeth() {
        const pathLeft = svgPaths.lineTo(5.1, 2.6) +
    ...
    makeStartHat() {
        ...
        const pathRtl = svgPaths.moveBy(25, -8.7) +
            svgPaths.curve('c', [
...
//# sourceMappingURL=highlight_constants.js.map