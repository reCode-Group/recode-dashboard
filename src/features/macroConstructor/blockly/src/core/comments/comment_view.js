...
export class CommentView {
     ...
     */
    createTopBar(svgRoot, workspace) {
        ...
        const deleteIcon = dom.createSvgElement(Svg.IMAGE, {
            'class': 'blocklyDeleteIcon',
            'href': `${workspace.options.pathToMedia}delete-icon.svg`,
        }, topBarGroup);
        const foldoutIcon = dom.createSvgElement(Svg.IMAGE, {
        ...
        //   to fix this.
        browserEvents.conditionalBind(foldoutIcon, 'pointerdown', this, this.onFoldoutDown);
        browserEvents.conditionalBind(deleteIcon, 'pointerdown', this, this.onDeleteDown);
        return {
            topBarGroup,
            topBarBackground,
            deleteIcon,
     ...
     */
    createTextArea(svgRoot) {
        const foreignObject = dom.createSvgElement(Svg.FOREIGNOBJECT, {
    ...
    setSizeWithoutFiringEvents(size) {
        ...
        size = Size.max(size, this.calcMinSize(topBarSize, foldoutSize, deleteSize));
        this.size = size;
        ...
        this.updateTextPreviewSize(size, topBarSize, textPreviewSize, deleteSize, resizeSize);
     ...
     */
    calcMinSize(topBarSize, foldoutSize, deleteSize) {
    ...
    updateTextAreaSize(size, topBarSize) {
     ...
     */
    updateDeleteIconPosition(size, topBarSize, deleteSize) {
        const deleteMargin = this.calcDeleteMargin(topBarSize, deleteSize);
        this.deleteIcon.setAttribute('y', `${deleteMargin}`);
        this.deleteIcon.setAttribute('x', `${size.width - deleteSize.width - deleteMargin}`);
    }
    /**
     * Updates the position of the foldout icon elements to reflect the new size.
     */
    updateFoldoutIconPosition(topBarSize, foldoutSize) {
        const foldoutMargin = this.calcFoldoutMargin(topBarSize, foldoutSize);
     ...
     */
    updateTextPreviewSize(size, topBarSize, textPreviewSize, deleteSize, foldoutSize) {
        const textPreviewMargin = (topBarSize.height - textPreviewSize.height) / 2;
        const deleteMargin = this.calcDeleteMargin(topBarSize, deleteSize);
        const foldoutMargin = this.calcFoldoutMargin(topBarSize, foldoutSize);
        const textPreviewWidth = size.width -
            foldoutSize.width -
            foldoutMargin * 2 -
            deleteSize.width -
            deleteMargin * 2;
        this.textPreview.setAttribute('x', `${foldoutSize.width + foldoutMargin * 2 * (this.workspace.RTL ? -1 : 1)}`);
