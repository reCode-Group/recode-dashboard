...
export class FocusManager {
    ...
    unregisterTree(tree) {
        ...
        if (focusedNode)
            this.removeHighlight(focusedNode);
    ...
    focusNode(focusableNode) {
        this.ensureManagerIsUnlocked();
        if (this.focusedNode === focusableNode)
        ...
        const matchedNode = FocusableTreeTraverser.findFocusableNodeFor(focusableNode.getFocusableElement(), nextTree);
    ...
    takeEphemeralFocus(focusableElement) {
        this.ensureManagerIsUnlocked();
        if (this.currentlyHoldsEphemeralFocus) {
            throw Error(`Attempted to take ephemeral focus when it's already held, ` +
        ...
        return () => {
            if (hasFinishedEphemeralFocus) {
                throw Error(`Attempted to finish ephemeral focus twice for element: ` +
                    `${focusableElement}.`);
            }
    ...
    ensureManagerIsUnlocked() {
        if (this.lockFocusStateChanges) {
            throw Error('FocusManager state changes cannot happen in a tree/node focus/blur ' +
                'callback.');
        }
    }
    /**
 ...
 */
FocusManager.ACTIVE_FOCUS_NODE_CSS_CLASS_NAME = 'blocklyActiveFocus';
/**
 * The CSS class assigned to IFocusableNode elements that presently have
 ...
 */
FocusManager.PASSIVE_FOCUS_NODE_CSS_CLASS_NAME = 'blocklyPassiveFocus';
