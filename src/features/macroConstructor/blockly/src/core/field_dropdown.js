...
export class FieldDropdown extends Field {
    ...
    createTextArrow_() {
        ...
        this.arrow.appendChild(document.createTextNode(this.getSourceBlock()?.RTL
    ...
    createSVGArrow_() {
        this.svgArrow = dom.createSvgElement(Svg.IMAGE, {
            'height': this.getConstants().FIELD_DROPDOWN_SVG_ARROW_SIZE + 'px',
            'width': this.getConstants().FIELD_DROPDOWN_SVG_ARROW_SIZE + 'px',
        }, this.fieldGroup_);
        this.svgArrow.setAttributeNS(dom.XLINK_NS, 'xlink:href', this.getConstants().FIELD_DROPDOWN_SVG_ARROW_DATAURI);
    }
    /**
    ...
    showEditor_(e) {
        ...
        if (this.getConstants().FIELD_DROPDOWN_COLOURED_DIV) {
            ...
            const borderColour = this.sourceBlock_.getColourTertiary();
     ...
     */
    renderSelectedImage(imageJson) {
        const block = this.getSourceBlock();
        if (!block) {
            throw new UnattachedFieldError();
        }
        this.imageElement.style.display = '';
        this.imageElement.setAttributeNS(dom.XLINK_NS, 'xlink:href', imageJson.src);
        ...
        const height = Math.max(hasBorder ? this.getConstants().FIELD_DROPDOWN_BORDER_RECT_HEIGHT : 0, imageHeight + FieldDropdown.IMAGE_Y_PADDING);
        const xPadding = hasBorder
            ? this.getConstants().FIELD_BORDER_RECT_X_PADDING
            : 0;
        let arrowWidth = 0;
        if (this.svgArrow) {
            arrowWidth = this.positionSVGArrow(imageWidth + xPadding, height / 2 - this.getConstants().FIELD_DROPDOWN_SVG_ARROW_SIZE / 2);
        }
        else {
            arrowWidth = dom.getTextWidth(this.arrow);
        }
    ...
    renderSelectedText() {
        ...
        let arrowWidth = 0;
        if (this.svgArrow) {
            arrowWidth = this.positionSVGArrow(textWidth + xPadding, height / 2 - this.getConstants().FIELD_DROPDOWN_SVG_ARROW_SIZE / 2);
        }
    ...
    getText_() {
        ...
        console.warn("Can't get text for existing dropdown option. If " +
     ...
     */
    applyTrim(options, prefixLength, suffixLength) {
    ...
    validateOptions(options) {
        ...
        for (let i = 0; i < options.length; i++) {
            ...
            if (!Array.isArray(option) && option !== FieldDropdown.SEPARATOR) {
                foundError = true;
                console.error(`Invalid option[${i}]: Each FieldDropdown option must be an array or
          the string literal 'separator'. Found: ${option}`);
            }
            else if (typeof option[1] !== 'string') {
                foundError = true;
                console.error(`Invalid option[${i}]: Each FieldDropdown option id must be a string. 
          Found ${option[1]} in: ${option}`);
                ...
                foundError = true;
                console.error(`Invalid option[${i}]: Each FieldDropdown option must have a string 
 ...
 */
FieldDropdown.SEPARATOR = 'separator';
FieldDropdown.ARROW_CHAR = '▾';
/**
...
//# sourceMappingURL=field_dropdown.js.map