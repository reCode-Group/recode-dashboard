...
export class ZoomControls {
    ...
    constructor(workspace) {
        ...
        this.zoomInGroup = null;
        /** The zoom out svg <g> element. */
        this.zoomOutGroup = null;
        /** The zoom reset svg <g> element. */
        this.zoomResetGroup = null;
    ...
    position(metrics, savedPositions) {
        ...
        const cornerPosition = uiPosition.getCornerOppositeToolbox(this.workspace, metrics);
        let height = this.SMALL_SPACING + 2 * this.HEIGHT;
        if (this.zoomResetGroup) {
            height += this.LARGE_SPACING + this.HEIGHT;
        }
        const startRect = uiPosition.getStartPositionRect(cornerPosition, new Size(this.WIDTH, height), this.MARGIN_HORIZONTAL, this.MARGIN_VERTICAL, metrics, this.workspace);
        ...
        const positionRect = uiPosition.bumpPositionRect(startRect, this.MARGIN_VERTICAL, bumpDirection, savedPositions);
        if (verticalPosition === uiPosition.verticalPosition.TOP) {
            const zoomInTranslateY = this.SMALL_SPACING + this.HEIGHT;
            this.zoomInGroup?.setAttribute('transform', 'translate(0, ' + zoomInTranslateY + ')');
            if (this.zoomResetGroup) {
                const zoomResetTranslateY = zoomInTranslateY + this.LARGE_SPACING + this.HEIGHT;
                this.zoomResetGroup.setAttribute('transform', 'translate(0, ' + zoomResetTranslateY + ')');
            }
        }
        else {
            const zoomInTranslateY = this.zoomResetGroup
                ...
                : 0;
            this.zoomInGroup?.setAttribute('transform', 'translate(0, ' + zoomInTranslateY + ')');
            const zoomOutTranslateY = zoomInTranslateY + this.SMALL_SPACING + this.HEIGHT;
            this.zoomOutGroup?.setAttribute('transform', 'translate(0, ' + zoomOutTranslateY + ')');
        }
    ...
    createZoomOutSvg(rnd) {
        /* This markup will be generated and added to the .svgGroup:
            ...
            */
        this.zoomOutGroup = dom.createSvgElement(Svg.G, { 'class': 'blocklyZoom blocklyZoomOut' }, this.svgGroup);
        const clip = dom.createSvgElement(Svg.CLIPPATH, { 'id': 'blocklyZoomoutClipPath' + rnd }, this.zoomOutGroup);
        dom.createSvgElement(Svg.RECT, {
            'width': 32,
            'height': 32,
        }, clip);
        const zoomoutSvg = dom.createSvgElement(Svg.IMAGE, {
            'width': SPRITE.width,
        ...
        zoomoutSvg.setAttributeNS(dom.XLINK_NS, 'xlink:href', this.workspace.options.pathToMedia + SPRITE.url);
        // Attach listener.
        this.boundEvents.push(browserEvents.conditionalBind(this.zoomOutGroup, 'pointerdown', null, this.zoom.bind(this, -1)));
    }
    /**
    ...
    createZoomInSvg(rnd) {
        /* This markup will be generated and added to the .svgGroup:
            ...
            */
        this.zoomInGroup = dom.createSvgElement(Svg.G, { 'class': 'blocklyZoom blocklyZoomIn' }, this.svgGroup);
        const clip = dom.createSvgElement(Svg.CLIPPATH, { 'id': 'blocklyZoominClipPath' + rnd }, this.zoomInGroup);
        dom.createSvgElement(Svg.RECT, {
            'width': 32,
            'height': 32,
        }, clip);
        const zoominSvg = dom.createSvgElement(Svg.IMAGE, {
            'width': SPRITE.width,
            'height': SPRITE.height,
            'x': -32,
            'y': -92,
        ...
        zoominSvg.setAttributeNS(dom.XLINK_NS, 'xlink:href', this.workspace.options.pathToMedia + SPRITE.url);
        // Attach listener.
        this.boundEvents.push(browserEvents.conditionalBind(this.zoomInGroup, 'pointerdown', null, this.zoom.bind(this, 1)));
    }
    /**
    ...
    createZoomResetSvg(rnd) {
        /* This markup will be generated and added to the .svgGroup:
            ...
            */
        this.zoomResetGroup = dom.createSvgElement(Svg.G, { 'class': 'blocklyZoom blocklyZoomReset' }, this.svgGroup);
        const clip = dom.createSvgElement(Svg.CLIPPATH, { 'id': 'blocklyZoomresetClipPath' + rnd }, this.zoomResetGroup);
        dom.createSvgElement(Svg.RECT, { 'width': 32, 'height': 32 }, clip);
        const zoomresetSvg = dom.createSvgElement(Svg.IMAGE, {
            'width': SPRITE.width,
            'height': SPRITE.height,
            'y': -92,
            'clip-path': 'url(#blocklyZoomresetClipPath' + rnd + ')',
        }, this.zoomResetGroup);
        zoomresetSvg.setAttributeNS(dom.XLINK_NS, 'xlink:href', this.workspace.options.pathToMedia + SPRITE.url);
        // Attach event listeners.
        this.boundEvents.push(browserEvents.conditionalBind(this.zoomResetGroup, 'pointerdown', null, this.resetZoom.bind(this)));
    }
    /**
    ...
    fireZoomEvent() {
        const uiEvent = new (eventUtils.get(EventType.CLICK))(null, this.workspace.id, 'zoom_controls');
...
`);
//# sourceMappingURL=zoom_controls.js.map