...
export class Gesture {
    ...
    constructor(e, creatorWorkspace) {
        this.creatorWorkspace = creatorWorkspace;
        /**
         ...
         * clientX/Y).
         */
        this.mouseDownXY = new Coordinate(0, 0);
        /**
         ...
         * is in progress.
         */
        this.workspaceDragger = null;
        /** Whether the gesture is dragging or not. */
        this.dragging = false;
        /** The flyout a gesture started in, if any. */
        this.flyout = null;
        /** Boolean for sanity-checking that some code is only called once. */
        this.calledUpdateIsDragging = false;
        /** Boolean for sanity-checking that some code is only called once. */
         ...
         * the dropdown.
         */
        this.currentDropdownOwner = null;
     ...
     */
    updateDragDelta(currentXY) {
        this.currentDragDeltaXY = Coordinate.difference(currentXY, this.mouseDownXY);
    ...
    updateIsDragging(e) {
        if (!this.startWorkspace_) {
            throw new Error('Cannot update dragging because the start workspace is undefined');
        }
    ...
    createDragger(draggable, workspace) {
        const DraggerClass = registry.getClassFromOptions(registry.Type.BLOCK_DRAGGER, this.creatorWorkspace.options, true);
    ...
    doStart(e) {
        if (!this.startWorkspace_) {
            throw new Error('Cannot start the touch gesture becauase the start ' +
     ...
     */
    bindMouseEvents(e) {
        this.boundEvents.push(browserEvents.conditionalBind(document, 'pointerdown', null, this.handleStart.bind(this), 
        /* opt_noCaptureIdentifier */ true));
        this.boundEvents.push(browserEvents.conditionalBind(document, 'pointermove', null, this.handleMove.bind(this), 
        /* opt_noCaptureIdentifier */ true));
        this.boundEvents.push(browserEvents.conditionalBind(document, 'pointerup', null, this.handleUp.bind(this), 
        /* opt_noCaptureIdentifier */ true));
        e.preventDefault();
        e.stopPropagation();
    }
    ...
    handlePinch(e) {
        ...
        if (this.previousScale > 0 && this.previousScale < Infinity) {
            ...
            if (!this.startWorkspace_) {
                throw new Error('Cannot handle a pinch because the start workspace ' + 'is undefined');
            }
            const workspace = this.startWorkspace_;
            const position = browserEvents.mouseToSvg(e, workspace.getParentSvg(), workspace.getInverseScreenCTM());
     ...
     */
    fireWorkspaceClick(ws) {
        eventUtils.fire(new (eventUtils.get(EventType.CLICK))(null, ws.id, 'workspace'));
    }
    /**
     ...
     * @internal
     */
    handleCommentStart(e, comment) {
        if (this.gestureHasStarted) {
            throw Error('Tried to call gesture.handleCommentStart, ' +
                'but the gesture had already been started.');
        }
    ...
    doFieldClick() {
        if (!this.startField) {
            throw new Error('Cannot do a field click because the start field is undefined');
        }
    ...
    doIconClick() {
        if (!this.startIcon) {
            throw new Error('Cannot do an icon click because the start icon is undefined');
        }
        this.bringBlockToFront();
    ...
    doBlockClick() {
        ...
        if (this.flyout && this.flyout.autoClose) {
            if (!this.targetBlock) {
                throw new Error('Cannot do a block click because the target block is ' + 'undefined');
            }
            ...
            const event = new (eventUtils.get(EventType.CLICK))(this.startBlock, this.startWorkspace_.id, 'block');
    ...
    setStartIcon(icon) {
        if (this.gestureHasStarted) {
            throw Error('Tried to call gesture.setStartIcon, ' +
                'but the gesture had already been started.');
        }
        if (!this.startIcon)
            this.startIcon = icon;
    ...
    isFieldClick() {
        if (!this.startField)
            return false;
        return (this.startField.isClickable() &&
            !this.hasExceededDragRadius &&
            (!this.flyout ||
    ...
    isIconClick() {
        if (!this.startIcon)
            return false;
        const handleInFlyout = !this.flyout ||
     ...
     */
    isWorkspaceClick() {
        const onlyTouchedWorkspace = !this.startBlock && !this.startBubble && !this.startField;
