...
let domParser = {
    parseFromString: function () {
        throw new Error('DOMParser was not found in the global scope and was not properly ' +
            'injected using injectDependencies');
    },
};
let xmlSerializer = {
    serializeToString: function () {
        throw new Error('XMLSerializer was not foundin the global scope and was not properly ' +
            'injected using injectDependencies');
    },
};
/**
...
function sanitizeText(text) {
    return text.replace(INVALID_CONTROL_CHARS, (match) => `&#${match.charCodeAt(0)};`);
}
