 ...
 */
function tokenizeInterpolationInternal(message, parseInterpolationTokens, tokenizeNewlines) {
    const tokens = [];
                ...
                if (/[A-Z]\w*/i.test(rawKey)) {
                    ...
                    if (bklyKey && bklyKey in Msg) {
                        ...
                        if (typeof rawValue === 'string') {
                            ...
                            // end.
                            Array.prototype.push.apply(tokens, tokenizeInterpolationInternal(rawValue, parseInterpolationTokens, tokenizeNewlines));
                        }
...
export function replaceMessageReferences(message) {
    ...
    const interpolatedResult = tokenizeInterpolationInternal(message, false, false);
 ...
 */
export function parseBlockColour(colour) {
    const dereferenced = typeof colour === 'string' ? replaceMessageReferences(colour) : colour;
    const hue = Number(dereferenced);
    if (!isNaN(hue) && 0 <= hue && hue <= 360) {
        return {
            hue: hue,
            hex: colourUtils.hsvToHex(hue, colourUtils.getHsvSaturation(), colourUtils.getHsvValue() * 255),
        };
    }
    else {
