 ...
 * the block.
 */
export class MutatorIcon extends Icon {
    constructor(flyoutBlockTypes, sourceBlock) {
        super(sourceBlock);
        this.flyoutBlockTypes = flyoutBlockTypes;
    ...
    initView(pointerdownListener) {
        ...
        // Axle hole.
        dom.createSvgElement(Svg.CIRCLE, { 'class': 'blocklyIconShape', 'r': '2.7', 'cx': '8', 'cy': '8' }, this.svgRoot);
    ...
    async setBubbleVisible(visible) {
        ...
        if (visible) {
            this.miniWorkspaceBubble = new MiniWorkspaceBubble(this.getMiniWorkspaceConfig(), this.sourceBlock.workspace, this.getAnchorLocation(), this.getBubbleOwnerRect());
            this.applyColour();
            this.createRootBlock();
            this.addSaveConnectionsListener();
            this.miniWorkspaceBubble?.addWorkspaceChangeListener(this.createMiniWorkspaceChangeListener());
        }
        else {
            this.miniWorkspaceBubble?.dispose();
            this.miniWorkspaceBubble = null;
        ...
        eventUtils.fire(new (eventUtils.get(EventType.BUBBLE_OPEN))(this.sourceBlock, visible, 'mutator'));
    }
    ...
    createRootBlock() {
        if (!this.sourceBlock.decompose) {
            throw new Error('Blocks with mutator icons must include a decompose method');
        }
        ...
        const flyoutWidth = this.miniWorkspaceBubble?.getWorkspace()?.getFlyout()?.getWidth() ?? 0;
        this.rootBlock.moveBy(this.rootBlock.RTL ? -(flyoutWidth + WORKSPACE_MARGIN) : WORKSPACE_MARGIN, WORKSPACE_MARGIN);
    }
    ...
    recomposeSourceBlock() {
        ...
        if (!this.sourceBlock.compose) {
            throw new Error('Blocks with mutator icons must include a compose method');
        }
        ...
        if (oldExtraState !== newExtraState) {
            eventUtils.fire(new (eventUtils.get(EventType.BLOCK_CHANGE))(this.sourceBlock, 'mutation', null, oldExtraState, newExtraState));
        }
...
MutatorIcon.TYPE = IconType.MUTATOR;
/**
 ...
 */
MutatorIcon.WEIGHT = 1;
//# sourceMappingURL=mutator_icon.js.map