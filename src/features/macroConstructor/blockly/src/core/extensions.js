...
export function registerMutator(name, mixinObj, opt_helperFn, opt_blockList) {
    ...
    register(name, function () {
        if (hasMutatorDialog) {
            this.setMutator(new MutatorIcon(opt_blockList || [], this));
        }
        // Mixin the object.
        this.mixin(mixinObj);
        if (opt_helperFn) {
...
export function unregister(name) {
    if (isRegistered(name)) {
        ...
        console.warn('No extension mapping for name "' + name + '" found to unregister');
    }
}
/**
...
export function apply(name, block, isMutator) {
    ...
    if (isMutator) {
        ...
        if (!mutatorPropertiesMatch(mutatorProperties, block)) {
            throw Error('Error when applying extension "' +
                name +
                '": ' +
...
function checkNoMutatorProperties(mutationName, block) {
    ...
    if (properties.length) {
        throw Error('Error: tried to apply mutation "' +
            mutationName +
...
function checkXmlHooks(object, errorPrefix) {
    return checkHasFunctionPair(object.mutationToDom, object.domToMutation, errorPrefix + ' mutationToDom/domToMutation');
}
/**
...
function checkJsonHooks(object, errorPrefix) {
    return checkHasFunctionPair(object.saveExtraState, object.loadExtraState, errorPrefix + ' saveExtraState/loadExtraState');
}
/**
...
function checkMutatorDialog(object, errorPrefix) {
    return checkHasFunctionPair(object.compose, object.decompose, errorPrefix + ' compose/decompose');
}
/**
...
function checkDropdownOptionsInTable(block, dropdownName, lookupTable) {
    ...
    for (const option of options) {
        ...
        if (lookupTable[key] === undefined) {
            console.warn(`No tooltip mapping for value ${key} of field ` +
 ...
 * @returns The extension function.
 */
export function buildTooltipWithFieldText(msgTemplate, fieldName) {
    return function () {
        this.setTooltip(function () {
