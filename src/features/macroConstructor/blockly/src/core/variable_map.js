...
export class VariableMap {
     ...
     *  workspace yet.
     */
    constructor(workspace, potentialMap = false) {
    ...
    changeVariableType(variable, newType) {
        ...
        const newTypeVariables = this.variableMap.get(newType) ??
            new Map();
    ...
    renameVariableById(id, newName) {
        deprecation.warn('VariableMap.renameVariableById', 'v12', 'v13', 'VariableMap.renameVariable');
    ...
    renameVariableAndUses(variable, newName, blocks) {
        if (!this.potentialMap) {
            eventUtils.fire(new (eventUtils.get(EventType.VAR_RENAME))(variable, newName));
        }
     ...
     * @param blocks The list of all blocks in the workspace.
     */
    renameVariableWithConflict(variable, newName, conflictVar, blocks) {
        const type = variable.getType();
    ...
    createVariable(name, opt_type, opt_id) {
        ...
        const type = opt_type || '';
        const VariableModel = registry.getObject(registry.Type.VARIABLE_MODEL, registry.DEFAULT, true);
        if (!VariableModel) {
    ...
    addVariable(variable) {
        const type = variable.getType();
        if (!this.variableMap.has(type)) {
            this.variableMap.set(type, new Map());
        }
    ...
    deleteVariable(variable) {
        ...
        try {
            ...
            if (!variables || !variables.has(variable.getId()))
                return;
    ...
    deleteVariableById(id) {
        deprecation.warn('VariableMap.deleteVariableById', 'v12', 'v13', 'Blockly.Variables.deleteVariable');
        const variable = this.getVariableById(id);
        if (variable) {
    ...
    getVariable(name, opt_type) {
        ...
        if (!variables)
            return null;
        return ([...variables.values()].find((variable) => Names.equals(variable.getName(), name)) ?? null);
    }
    /**
     ...
     */
    getAllVariableNames() {
        deprecation.warn('VariableMap.getAllVariableNames', 'v12', 'v13', 'Blockly.Variables.getAllVariables');
        const names = [];
        for (const variables of this.variableMap.values()) {
            for (const variable of variables.values()) {
    ...
    getVariableUsesById(id) {
        deprecation.warn('VariableMap.getVariableUsesById', 'v12', 'v13', 'Blockly.Variables.getVariableUsesById');
        return getVariableUsesById(this.workspace, id);
    }
}
registry.register(registry.Type.VARIABLE_MAP, registry.DEFAULT, VariableMap);
//# sourceMappingURL=variable_map.js.map