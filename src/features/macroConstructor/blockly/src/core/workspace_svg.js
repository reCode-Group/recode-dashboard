...
export class WorkspaceSvg extends Workspace {
    ...
    constructor(options) {
         ...
         */
        this.injectionDiv = null;
        /**
         * Last known position of the page scroll.
         * This is used to determine whether we have recalculated screen coordinate
         * stuff since the page scrolled.
         */
        this.lastRecordedPageScroll = null;
        /**
         ...
         */
        this.configureContextMenu = null;
        /**
        ...
        this.keyboardMoveInProgress = false; // TODO(#8960): Delete this.
        ...
        const MetricsManagerClass = registry.getClassFromOptions(registry.Type.METRICS_MANAGER, options, true);
         ...
         */
        this.audioManager = new WorkspaceAudio(options.parentWorkspace);
        ...
        if (Variables && Variables.internalFlyoutCategory) {
            this.registerToolboxCategoryCallback(Variables.CATEGORY_NAME, Variables.internalFlyoutCategory);
        }
        if (VariablesDynamic && VariablesDynamic.internalFlyoutCategory) {
            this.registerToolboxCategoryCallback(VariablesDynamic.CATEGORY_NAME, VariablesDynamic.internalFlyoutCategory);
        }
        if (Procedures && Procedures.internalFlyoutCategory) {
            this.registerToolboxCategoryCallback(Procedures.CATEGORY_NAME, Procedures.internalFlyoutCategory);
        ...
        this.renderer = blockRendering.init(this.options.renderer || 'geras', this.getTheme(), this.options.rendererOverrides ?? undefined);
        /**
    ...
    refreshTheme() {
        if (this.svgGroup_) {
            ...
            this.renderer.refreshDom(this.svgGroup_, this.getTheme(), isParentWorkspace ? this.getInjectionDiv() : undefined);
        }
        // Update all blocks in workspace that have a style name.
        this.updateBlockStyles(this.getAllBlocks(false).filter((block) => !!block.getStyleName()));
        ...
        const event = new (eventUtils.get(EventType.THEME_CHANGE))(this.getTheme().name, this.id);
     ...
     */
    createDom(opt_backgroundClass, injectionDiv) {
        if (!this.injectionDiv) {
        ...
        if (opt_backgroundClass) {
            this.svgBackground_ = dom.createSvgElement(Svg.RECT, { 'height': '100%', 'width': '100%', 'class': opt_backgroundClass }, this.svgGroup_);
            if (opt_backgroundClass === 'blocklyMainBackground' && this.grid) {
                this.svgBackground_.style.fill = 'var(--blocklyGridPattern)';
            }
            else {
                this.themeManager_.subscribe(this.svgBackground_, 'workspaceBackgroundColour', 'fill');
            }
        }
        ...
        if (!this.isFlyout) {
            browserEvents.conditionalBind(this.svgGroup_, 'pointerdown', this, this.onMouseDown, false);
            ...
            browserEvents.conditionalBind(this.svgGroup_, 'wheel', this, this.onMouseWheel);
        }
        ...
        if (CursorClass)
            this.markerManager.setCursor(new CursorClass(this));
        const isParentWorkspace = this.options.parentWorkspace === null;
        this.renderer.createDom(this.svgGroup_, this.getTheme(), isParentWorkspace ? this.getInjectionDiv() : undefined);
    ...
    addFlyout(tagName) {
        ...
        if (this.horizontalLayout) {
            const HorizontalFlyout = registry.getClassFromOptions(registry.Type.FLYOUTS_HORIZONTAL_TOOLBOX, this.options, true);
            this.flyout = new HorizontalFlyout(workspaceOptions);
        }
        else {
            const VerticalFlyout = registry.getClassFromOptions(registry.Type.FLYOUTS_VERTICAL_TOOLBOX, this.options, true);
    ...
    resize() {
        ...
        const positionables = this.componentManager.getComponents(ComponentManager.Capability.POSITIONABLE, true);
    ...
    maybeFireViewportChangeEvent() {
        ...
        const event = new (eventUtils.get(EventType.VIEWPORT_CHANGE))(top, left, scale, this.id, this.oldScale);
        this.oldScale = scale;
        this.oldTop = top;
        this.oldLeft = left;
        eventUtils.fire(event);
    ...
    recordDragTargets() {
        const dragTargets = this.componentManager.getComponents(ComponentManager.Capability.DRAG_TARGET, true);
        this.dragTargetAreas = [];
     ...
     */
    newComment(id) {
        throw new Error('The implementation of newComment should be ' +
            'monkey-patched in by blockly.ts');
    }
    /* eslint-enable */
    /**
     ...
     */
    setKeyboardMoveInProgress(inProgress) {
    ...
    onMouseWheel(e) {
        ...
        const canWheelZoom = this.options.zoomOptions && this.options.zoomOptions.wheel;
        const canWheelMove = this.options.moveOptions && this.options.moveOptions.wheel;
        ...
        if (canWheelZoom && (e.ctrlKey || commandKey || !canWheelMove)) {
            ...
            const position = browserEvents.mouseToSvg(e, this.getParentSvg(), this.getInverseScreenCTM());
    ...
    getBlocksBoundingBox() {
        ...
        for (let i = 1; i < topElements.length; i++) {
            ...
            if (topElement.isInsertionMarker &&
                topElement.isInsertionMarker()) {
                continue;
            }
    ...
    cleanUp() {
        ...
        const getNextIntersectingImmovableBlock = function (rect) {
    ...
    showContextMenu(e) {
        ...
        const menuOptions = ContextMenuRegistry.registry.getContextMenuOptions({ workspace: this, focusedNode: this }, e);
    ...
    getAbsoluteScale() {
        ...
        const computeScale = (workspace, scale) => {
            ...
            // hierarchy.
            return computeScale(workspace.options.parentWorkspace, scale * getLocalScale(workspace));
        };
    ...
    scroll(x, y) {
        ...
        const maxYDisplacement = Math.max(0, metrics.scrollHeight - metrics.viewHeight);
        ...
        if (this.scrollbar) {
            ...
            this.scrollbar.set(-(x + metrics.scrollLeft), -(y + metrics.scrollTop), false);
        }
     ...
     */
    getAllBlocks(ordered = false) {
        return super.getAllBlocks(ordered);
    }
    /**
     ...
     */
    addTopBlock(block) {
        this.addTopBoundedElement(block);
    ...
    removeTopBlock(block) {
        this.removeTopBoundedElement(block);
    ...
    getTopBoundedElements() {
        return new Array().concat(this.topBoundedElements);
    }
    /**
    ...
    hideComponents(onlyClosePopups = false) {
        const autoHideables = this.getComponentManager().getComponents(ComponentManager.Capability.AUTOHIDEABLE, true);
        autoHideables.forEach((autoHideable) => autoHideable.autoHide(onlyClosePopups));
    }
    /**
     ...
     */
    static setTopLevelWorkspaceMetrics(xyRatio) {
        const metrics = this.getMetrics();
        if (typeof xyRatio.x === 'number') {
            this.scrollX = -(metrics.scrollLeft +
                (metrics.scrollWidth - metrics.viewWidth) * xyRatio.x);
        }
        if (typeof xyRatio.y === 'number') {
            this.scrollY = -(metrics.scrollTop +
     ...
     */
    scrollBoundsIntoView(bounds, padding = 10) {
        if (Gesture.inProgress()) {
        ...
        const viewport = new Rect(rawViewport.top, rawViewport.top + rawViewport.height, rawViewport.left, rawViewport.left + rawViewport.width);
        ...
        if (bounds.left < viewport.left) {
            ...
            deltaX = this.RTL
                ...
                : Math.max(viewport.right - bounds.right, viewport.left - bounds.left);
        }
        if (bounds.top < viewport.top) {
            deltaY = viewport.top - bounds.top;
        }
        else if (bounds.bottom > viewport.bottom) {
            deltaY = Math.max(viewport.bottom - bounds.bottom, viewport.top - bounds.top);
        }
        deltaX *= scale;
        deltaY *= scale;
    ...
    lookUpFocusableNode(id) {
        ...
        const block = this.getAllBlocks(false).find((block) => block.getFocusableElement().id === id);
        if (block)
            return block;
    ...
    onTreeBlur(nextTree) {
        ...
        if (nextTree && this.isFlyout && this.targetWorkspace) {
            ...
            if (flyout && nextTree === flyout)
                return;
            if (toolbox && nextTree === toolbox)
                return;
            if (toolbox)
                toolbox.clearSelection();
            if (flyout && isAutoHideable(flyout))
...
//# sourceMappingURL=workspace_svg.js.map