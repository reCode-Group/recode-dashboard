 ...
 */
export function setBoundsElement(boundsElem) {
 ...
 */
export function setColour(backgroundColour, borderColour) {
 ...
 */
export function showPositionedByBlock(field, block, opt_onHide, opt_secondaryYOffset) {
    return showPositionedByRect(getScaledBboxOfBlock(block), field, opt_onHide, opt_secondaryYOffset);
}
/**
 * Shortcut to show and place the drop-down with positioning determined
 ...
 * @returns True if the menu rendered below block; false if above.
 */
export function showPositionedByField(field, opt_onHide, opt_secondaryYOffset) {
    positionToField = true;
    return showPositionedByRect(getScaledBboxOfField(field), field, opt_onHide, opt_secondaryYOffset);
}
/**
 ...
 * @returns True if the menu rendered below block; false if above.
 */
function showPositionedByRect(bBox, field, opt_onHide, opt_secondaryYOffset, manageEphemeralFocus = true) {
    ...
    setBoundsElement(workspace.getParentSvg().parentNode);
    return show(field, sourceBlock.RTL, primaryX, primaryY, secondaryX, secondaryY, manageEphemeralFocus, opt_onHide);
}
/**
 ...
 * @internal
 */
export function show(newOwner, rtl, primaryX, primaryY, secondaryX, secondaryY, manageEphemeralFocus, opt_onHide) {
    owner = newOwner;
...
const internal = {
    ...
    getBoundsInfo: function () {
        const boundPosition = style.getPageOffset(boundsElement);
        const boundSize = style.getSize(boundsElement);
        return {
     ...
     *     and arrow.
     */
    getPositionMetrics: function (primaryX, primaryY, secondaryX, secondaryY) {
 ...
 *     and arrow.
 */
function getPositionBelowMetrics(primaryX, primaryY, boundsInfo, divSize) {
    const xCoords = getPositionX(primaryX, boundsInfo.left, boundsInfo.right, divSize.width);
 ...
 *     and arrow.
 */
function getPositionAboveMetrics(secondaryX, secondaryY, boundsInfo, divSize) {
    const xCoords = getPositionX(secondaryX, boundsInfo.left, boundsInfo.right, divSize.width);
 ...
 *     and arrow.
 */
function getPositionTopOfPageMetrics(sourceX, boundsInfo, divSize) {
    const xCoords = getPositionX(sourceX, boundsInfo.left, boundsInfo.right, divSize.width);
 ...
 * @internal
 */
export function getPositionX(sourceX, boundsLeft, boundsRight, divWidth) {
    let divX = sourceX;
    ...
    relativeArrowX = math.clamp(horizPadding, relativeArrowX, divWidth - horizPadding - ARROW_SIZE);
 ...
 */
export function hideIfOwner(divOwner, opt_withoutAnimation) {
    if (owner === divOwner) {
 ...
 * @returns True if the menu rendered at the primary origin point.
 */
function positionInternal(primaryX, primaryY, secondaryX, secondaryY) {
    const metrics = internal.getPositionMetrics(primaryX, primaryY, secondaryX, secondaryY);
    // Update arrow CSS.
...
//# sourceMappingURL=dropdowndiv.js.map