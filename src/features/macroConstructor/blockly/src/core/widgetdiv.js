 ...
 *     to true.
 */
export function show(newOwner, rtl, newDispose, workspace, manageEphemeralFocus = true) {
    hide();
    owner = newOwner;
    dispose = newDispose;
    ...
    if (!workspace && newOwner instanceof Field) {
        ...
        workspace = newOwner.getSourceBlock()?.workspace;
    }
    ownerWorkspace = workspace ?? null;
    const rendererWorkspace = workspace ?? common.getMainWorkspace();
 ...
 */
export function hideIfOwnerIsInWorkspace(workspace) {
 ...
 * dependency.
 */
function isRepositionable(item) {
