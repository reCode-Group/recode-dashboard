...
export class ConnectionNavigationPolicy {
    ...
    getParent(current) {
        ...
        return (this.getParentConnection(topBlock)?.targetConnection?.getParentInput()
            ?.connection ?? topBlock);
    }
    /**
    ...
    getPreviousSibling(current) {
        if (current.getParentInput()) {
            ...
            for (let i = curIdx; i >= 0; i--) {
                ...
                const fieldRow = input.fieldRow;
                if (fieldRow.length)
                    return fieldRow[fieldRow.length - 1];
            }
            return null;
        }
        else if (current.type === ConnectionType.PREVIOUS_STATEMENT ||
            current.type === ConnectionType.OUTPUT_VALUE) {
            const previousConnection = current.targetConnection && !current.targetConnection.getParentInput()
            ...
            if (!previousConnection &&
                ...
                return (previousRootBlock.lastConnectionInStack(false) ?? previousRootBlock);
            }
...
//# sourceMappingURL=connection_navigation_policy.js.map