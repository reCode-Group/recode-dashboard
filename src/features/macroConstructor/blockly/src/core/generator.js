...
export class CodeGenerator {
    ...
    blockToCode(block, opt_thisOnly) {
        if (this.isInitialized === false) {
            console.warn('CodeGenerator init was not called before blockToCode was called.');
        }
        if (!block) {
            return '';
        }
        ...
        if (typeof func !== 'function') {
            throw Error(`${this.name_} generator does not know how to generate code ` +
    ...
    valueToCode(block, name, outerOrder) {
        ...
        if (!Array.isArray(tuple)) {
            throw TypeError(`Expecting tuple from value block: ${targetBlock.type} See ` +
    ...
    addLoopTrap(branch, block) {
        if (this.INFINITE_LOOP_TRAP) {
            branch =
                this.prefixLines(this.injectId(this.INFINITE_LOOP_TRAP, block), this.INDENT) + branch;
        }
        if (this.STATEMENT_SUFFIX && !block.suppressPrefixSuffix) {
            branch =
                this.prefixLines(this.injectId(this.STATEMENT_SUFFIX, block), this.INDENT) + branch;
        }
        if (this.STATEMENT_PREFIX && !block.suppressPrefixSuffix) {
            branch =
                branch +
                    this.prefixLines(this.injectId(this.STATEMENT_PREFIX, block), this.INDENT);
        }
        return branch;
    }
    /**
    ...
    provideFunction_(desiredName, code) {
        if (!this.definitions_[desiredName]) {
            const functionName = this.nameDB_.getDistinctName(desiredName, NameType.PROCEDURE);
    ...
    getName(nameOrId, type) {
        if (!this.nameDB_) {
            throw new Error('Name database is not defined. You must initialize `nameDB_` in your generator class and call `init` first.');
        }
