 ...
 */
export function workspaceToDom(workspace, skipId = false) {
    const treeXml = utilsXml.createElement('xml');
    const variablesElement = variablesToDom(Variables.allUsedVarModels(workspace));
...
export function saveWorkspaceComment(comment, skipId = false) {
    ...
    if (comment.getText())
        elem.textContent = comment.getText();
    if (comment.isCollapsed())
        elem.setAttribute('collapsed', 'true');
    if (!comment.isOwnEditable())
        elem.setAttribute('editable', 'false');
    if (!comment.isOwnMovable())
        elem.setAttribute('movable', 'false');
    if (!comment.isOwnDeletable())
        elem.setAttribute('deletable', 'false');
    return elem;
}
/**
...
export function blockToDomWithXY(block, opt_noId) {
    ...
    if (isElement(element)) {
        ...
        element.setAttribute('x', String(Math.round(block.workspace.RTL ? width - xy.x : xy.x)));
 ...
 */
function allFieldsToDom(block, element) {
...
export function blockToDom(block, opt_noId) {
    ...
    if (commentText) {
        ...
        if (location) {
            commentElement.setAttribute('x', `${block.workspace.RTL
    ...
    if (!block.isEnabled()) {
        ...
        element.setAttribute('disabled-reasons', Array.from(block.getDisabledReasons()).map(encodeURIComponent).join(','));
    }
 ...
 */
function cloneShadow(shadow, opt_noId) {
...
export function loadWorkspaceComment(elem, workspace) {
    ...
    if (!isNaN(w) && !isNaN(h))
        comment.setSize(new Size(w, h));
    if (elem.getAttribute('collapsed') === 'true')
        comment.setCollapsed(true);
    if (elem.getAttribute('editable') === 'false')
        comment.setEditable(false);
    if (elem.getAttribute('movable') === 'false')
        comment.setMovable(false);
    if (elem.getAttribute('deletable') === 'false')
...
export function appendDomToWorkspace(xml, workspace) {
    ...
    // Assume it is rendered so we can check.
    if (!workspace.getBlocksBoundingBox) {
 ...
 * @internal
 */
export function domToBlockInternal(xmlBlock, workspace) {
    ...
    if (eventUtils.isEnabled()) {
        const newVariables = Variables.getAddedVariables(workspace, variablesBeforeCreation);
 ...
 */
function mapSupportedXmlTags(xmlBlock) {
    const childNodeMap = {
        mutation: new Array(),
        comment: new Array(),
        data: new Array(),
 ...
 */
function applyMutationTagNodes(xmlChildren, block) {
 ...
 * @param block The block to apply the child nodes on.
 */
function applyCommentTagNodes(xmlChildren, block) {
    for (let i = 0; i < xmlChildren.length; i++) {
        const xmlChild = xmlChildren[i];
 ...
 * @param block The block to apply the child nodes on.
 */
function applyDataTagNodes(xmlChildren, block) {
    for (let i = 0; i < xmlChildren.length; i++) {
        const xmlChild = xmlChildren[i];
 ...
 * @param block The block to apply the child nodes on.
 */
function applyFieldTagNodes(xmlChildren, block) {
    for (let i = 0; i < xmlChildren.length; i++) {
        const xmlChild = xmlChildren[i];
 ...
 */
function findChildBlocks(xmlNode) {
 ...
 */
function applyInputTagNodes(xmlChildren, workspace, block, prototypeName) {
    for (let i = 0; i < xmlChildren.length; i++) {
        const xmlChild = xmlChildren[i];
        const nodeName = xmlChild.getAttribute('name');
        ...
        if (childBlockInfo.childBlockElement) {
            ...
            domToBlockHeadless(childBlockInfo.childBlockElement, workspace, input.connection, false);
        }
 ...
 * @param block The block to apply the child nodes on.
 */
function applyNextTagNodes(xmlChildren, workspace, block) {
    for (let i = 0; i < xmlChildren.length; i++) {
        const xmlChild = xmlChildren[i];
        const childBlockInfo = findChildBlocks(xmlChild);
        if (childBlockInfo.childBlockElement) {
            ...
            // Create child block.
            domToBlockHeadless(childBlockInfo.childBlockElement, workspace, block.nextConnection, true);
        }
        // Set shadow after so we don't create a shadow we delete immediately.
 ...
 * @returns The root block created.
 */
function domToBlockHeadless(xmlBlock, workspace, parentConnection, connectedToParentNext) {
    let block = null;
    ...
    if (disabled) {
        ...
        block.setDisabledReason(disabled === 'true' || disabled === 'disabled', MANUALLY_DISABLED);
    }
 ...
 */
function domToField(block, fieldName, xml) {
